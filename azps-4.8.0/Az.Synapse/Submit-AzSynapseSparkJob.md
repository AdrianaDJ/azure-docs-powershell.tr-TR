...
### Runparlak Jobparametersetname (varsayılan)
```
...
### Runparlak Jobbyparentobjectparameterset
```
...
## Tanım
**Submit-AzSynapseSparkJob** cmdlet 'ı bir Synapse Analytics Spark işi gönderir.
...
Bu komut bir Synapse çözümleme Spark işi gönderir.
...
Bu komut, SYNAPSE Analizi spark .NET işini gönderir.
...
Bu komut bir Synapse Analytics PySpark işi gönderir.

## PARAMETRELERINE

### -Commanddoğrtası
İş için isteğe bağlı bağımsız değişkenler. Örneğin, "--Iteration 10000--Timeout 20s"
...
### -Yürüttorsayısı
İş için belirtilen Spark havuzunda ayrılacak yürütme sayısı.
...
### -Yürüttorsize
İş için belirtilen Spark havuzunda tahsis edilen yürüticileri için kullanılacak çekirdek ve bellek sayısı.
...
### -MainClassName
Tam nitelikli tanımlayıcı veya ana tanım dosyasındaki ana sınıf.
Spark ve .NET Spark işi için gereklidir.
Örneğin, "org. Apache. spark. örnekler. parlak Pi"
...
### -MainDefinitionFile
İş için kullanılan ana dosya.
Örneğin, " abfss://filesystem@account.dfs.core.windows.net/mySpark.jar "
...
### -ReferenceFile
Ana tanım dosyasında başvuru için kullanılan ek dosyalar. Virgülle ayrılmış depolama URI listesi. Örneğin, " abfss://filesystem@account.dfs.core.windows.net/file1.txt , abfss://filesystem@account.dfs.core.windows.net/result/ "
