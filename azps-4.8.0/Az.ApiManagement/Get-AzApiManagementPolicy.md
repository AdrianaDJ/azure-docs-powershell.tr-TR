...
## Tanım
**Get-Azapsananagementpolicy** cmdlet 'i, belirtilen kapsam ilkesini alır.
...
### Örnek 5: RawXml biçiminde kiracı kapsam ilkesini edinme
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Bu komut, kiracı kapsam ilkesini XML dışında kaçan biçimde alır.
...
### -Apırevision
API düzeltme tanıtıcısı. Bu parametre isteğe bağlıdır. Belirtilmemişse, ilke geçerli etkin API düzeltmesidir.
...
Bu parametre için varsayılan değer "xml" olur.
