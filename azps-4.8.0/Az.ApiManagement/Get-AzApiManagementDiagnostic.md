...
## Tanım
**Get-Azapsananagementdiagnostic** , belirli bir kapsamda API Yönetim hizmetinde yapılandırılan tanılamaların ayrıntılarını alır.

## ÖRNEKLERDEN

### Örnek 1: kiracı kapsamında yapılandırılmış tüm tanılamayı alın.
```powershell
...
Bu komut, API Yönetim hizmetinde yapılandırılmış tüm tanılamayı alır.

### Örnek 2: API kapsamında yapılandırılmış tüm tanılamayı alma
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Bu komut API kapsamında yapılandırılmış tüm tanılamayı alır `echo-api`

### Örnek 3: bir kimlikle belirtilen API kapsam tanılaması 'nı alma
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Bu komut `applicationinsights` API 'de yapılandırılan tanılamaları alır `echo-api` .

## PARAMETRELERINE

### -Apııd
Var olan API tanıtıcısı.
Belirtilmişse API kapsam tanılaması döndürür.
...
Belirtilmişse ürün kapsam ilkesi döndürür.
Bu parametreler isteğe bağlıdır.
...
### -RESOURCEID
Bir tanılama veya API tanısı kolu kaynak tanımlayıcısı. Belirtilmişse tanımlayıcı tarafından tanılama bulmayı dener. Bu parametre gereklidir.
