...
### Örnek 1: taşıma koleksiyonuna kaynak ekleme.
```powershell
...
Belirtilen abonelikteki taşıma koleksiyonuna kaynak ekleme.
...
### -Bağımlılar
Kaynak bağımlılıklarını taşıma geçersiz kılmalarını alır veya ayarlar.
Oluşturmak için, BAĞıMLıDEĞER GEÇERSIZ kılma özellikleri için Notlar bölümüne bakın ve karma tablo oluşturun.
...
### -ExistingTargetId
Kaynağın varolan hedef ARM kimliğini alır veya ayarlar.
...
Kaynak türü.
Örneğin, değer Microsoft. COMPUTE/virtualMachines olabilir.
...
Hedef kaynak adını alır veya ayarlar.
...
### -SourceID
Kaynağın kaynak ARM kimliğini alır veya ayarlar.
...
### Microsoft. Azure. PowerShell. cmdlet. Kaynaktaşıyıcısı. modeller. Api20191001Preview. ımoveresource
...
BAĞıMLıNO geçersiz KıLMA <IMoveResourceDependencyOverride [] >: kaynak taşımayı taşıma geçersiz kılmalarını alır veya ayarlar.
  - `[Id <String>]`: Bağımlı kaynağın ARM KIMLIĞINI alır veya ayarlar.
  - `[TargetId <String>]`: MoveResource veya bağımlı kaynağın kaynak ARM KIMLIĞI 'nin kaynak kolu kimliğini alır veya ayarlar.
