...
## Tanım
**Add-Azanahtarvaultnetworkrule** cmdlet 'ı, IP adresleriyle veya ait oldukları sanal ağla belirlenen bir arayan kümesi için Anahtar Kasası erişimine izin verir veya erişimi kısıtlar. Kuralın erişim ilkesi aracılığıyla izin verilen diğer kullanıcılar, uygulamalar veya güvenlik grupları için erişimi kısıtlama olasılığı vardır.

Lütfen içindeki tüm IP aralığının `10.0.0.0–10.255.255.255` (özel IP adresleri) ağ kuralları eklemek için kullanılamayacağını unutmayın.
...
Bu komut, $myNetworkResId ile belirtilen sanal ağdan belirtilen IP adresine erişim izni vererek belirtilen kasaya bir ağ kuralı ekler.
