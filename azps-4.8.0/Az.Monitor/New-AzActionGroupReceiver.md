...
### -Automationrunbookalıcıdır
Automationrunbookalıcıoluşturma
...
Web kancaları 'nın gönderilmesi gereken URI
...
### -ConnectionID
Bu alıcının ISM bağlantı kimliği
...
### -Functionappresourceıd
işlev uygulaması RESOURCEID
...
### -Identifieruri
aad kimlik doğrulaması için tanımlayıcı URI 'si
...
### -Isglobalrunbook
Bu örneğin genel runbook olup olmadığını gösteren
...
### -ObjectID
aad kimlik doğrulaması için Web kancası uygulama nesnesi kimliği
...
### -Tenantıd
aad kimlik doğrulaması için Kiracı kimliği
...
### -Bilet yapılandırması
Bu alıcının ITSM anahtar yapılandırması
...
Genel uyarı şemasının kullanılıp kullanılmayacağını bayrak. Bu değer SMS, Azure Uygulama itme, ıTSM ve sesli recıevers için neglezererecektir.
...
### -
Bu alıcının itsm çalışma alanı kimliği
