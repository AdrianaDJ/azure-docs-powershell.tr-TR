...
### Örnek 1: Veriricks çalışma alanının etiketlerini güncelleştirir
```powershell
...
Bu komut, Veriricks çalışma alanının etiketlerini güncelleştirir.

### Örnek 2: Veriricks çalışma alanında şifrelemeyi etkinleştirme
```powershell
...
Databricks çalışma alanında şifrelemeyi etkinleştirmek üç adımdan yararlanır:
1.
Çalışma alanını `-PrepareEncryption` (henüz oluşturmadıysanız) güncelleştirin.
1.
`StorageAccountIdentityPrincipalId`Son adımın çıktısında bulun.
Sorumluya anahtar izinleri verin.
1.
Şifreleme anahtarıyla ilgili bilgileri doldurmak için çalışma alanını yeniden güncelleyin:
    - `-EncryptionKeySource`
    - `-EncryptionKeyVaultUri`
    - `-EncryptionKeyName`
    - `-EncryptionKeyVersion`

### Örnek 3: Veriricks çalışma alanında şifrelemeyi devre dışı bırakma
```powershell
...
Şifrelemeyi devre dışı bırakmak için, seçeneğini ayarlayın `-EncryptionKeySource` `'Default'` .
...
### -EncryptionKeySource
Şifreleme keySource (sağlayıcı).
Olası değerler (büyük/küçük harf duyarsız): varsayılan, Microsoft. Keykasa
