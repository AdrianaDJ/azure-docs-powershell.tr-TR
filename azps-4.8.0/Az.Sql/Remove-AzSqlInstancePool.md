...
## Tanım
**Remove-Azsqlınstancepool** cmdlet 'ı BIR Azure SQL örnek havuzunu kaldırır.

## ÖRNEKLERDEN

### Örnek 1: örnek havuzunu kaldırma
```powershell
PS C:\> Remove-AzSqlInstancePool -ResourceGroup resourcegroup01 -Name instancePool0
```

### Örnek 2: kaynak tanımlayıcısına göre örnek havuzunu kaldırma
```powershell
...
### Örnek 3: örnek havuz nesnesiyle örnek havuzu kaldırma
```powershell
...
Bu komut, instancePool0 adlı bir örnek havuzunu kaldırır.
...
### -InputObject
Kaldırılacak örnek havuzu nesnesi.
...
### -RESOURCEID
Kaldırılacak örnek havuzu kaynak kimliği.
