...
### Managedımagedağıtıcı (varsayılan)
```
...
### Örnek 1: yönetilen görüntü dağıtıcısı oluşturma
```powershell
...
Bu komut, yönetilen görüntü dağıtıcısı oluşturur.

### Örnek 2: VHD dağıtıcısı oluşturma
```powershell
...
Bu komut, bir VHD dağıtıcısı oluşturur.

### Örnek 3: paylaşılan görüntü dağıtıcısı oluşturma
```powershell
...
Bu komut paylaşılan görüntü dağıtıcısı oluşturur.

## PARAMETRELERINE

### -ArtifactTag
Dağıtımcı tarafından oluşturulduktan/güncelleştirildikten sonra yapıya uygulanacak Etiketler.
...
### -ExcludeFromLatest
Oluşturulan görüntü sürümünün en son dışında dışlanıp dışlanmayacağını belirten bayrak.
Varsayılanı (yanlış) kullanmak için atlayın.
...
### -Gallerımageıd
Paylaşılan görüntü Galerisi görüntüsünün kaynak kimliği.
...
### -ImageID
Yönetilen disk görüntüsünün kaynak kimliği.
...
### -Konum
Görüntü zaten varsa, resim için Azure konumu eşleşmelidir.
...
### -Managedımagedağıtıcı
Yönetilen disk görüntüsü olarak dağıtın.
...
### -ReplicationRegion
Yansımanın çoğaltılacağı bölgelerin listesi.
...
### -RunOutputName
İlişkili RunOutput için kullanılacak ad.
...
### -Sharedımagedağıtıcı
Paylaşılan görüntü Galerisi aracılığıyla dağıtın.
...
### -StorageAccountType
Paylaşılan Yansımayı depolamak için kullanılacak depolama hesabı türü.
Varsayılanı (Standard_LRS) kullanmak için atlayın.
...
### -Vhddağıtıcı
Depolama hesabında VHD ile dağıtın.
...
### Microsoft. Azure. PowerShell. cmdlet. ımagebuilder. modeller. Api20200214. ıımagetemplatedağıtıcı
