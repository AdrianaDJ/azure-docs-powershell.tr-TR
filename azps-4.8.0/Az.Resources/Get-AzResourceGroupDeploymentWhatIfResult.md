...
## Tanım
**Get-Azresourcegroupdeploymentwhaınresult** cmdlet 'i, belirtilen kaynak grubu kapsamındaki bir şablon dağıtımının ARM şablonunu What-If sonucunu alır. Gerçek kaynaklarda hiçbir değişiklik yapılmadan dağıtım uygulandığında hangi kaynakların güncelleştirileceğini gösteren değişikliklerin listesini döndürür. Döndürme sonucunun biçimini belirtmek için, *RESULTFORMAT* parametresini kullanın.

## ÖRNEKLERDEN

### Örnek 1: kaynak grup kapsamına bir What-If sonucu alma
```powershell
...
Bu komut, disk üzerinde özel bir şablon dosyası ve bir parametre dosyası kullanarak belirtilen kaynak grubu kapsamındaki bir What-If sonucu alır.
Komut, şablonun dağıtılacağı bir kaynak grubu belirtmek için *Resourcegroupname* parametresini kullanır.
...
### Örnek 2: kaynak grubu kapsamında yalnızca Resourceıdile What-If sonucu alma
```powershell
PS C:\> Get-AzResourceGroupDeploymentWhatIfResult `
    -DeploymentName "deploy-01" `
    -ResourceGroupName "myRG1" `
...
### -ExcludeChangeType
Virgülle ayrılmış kaynak değiştirme türleri What-If sonuçlarından dışlanır.
