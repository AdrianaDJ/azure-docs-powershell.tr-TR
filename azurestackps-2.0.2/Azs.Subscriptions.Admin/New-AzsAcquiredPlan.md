...
Abonelik planı oluşturma.

## PARAMETRELERINE

### -AcquiredPlanDefinition
Oluşturmak için, ACQUIREDPLANDEFINITION özellikleri için Notlar bölümüne bakın ve karma tablo oluşturun.
...
ACQUIREDPLANDEFINITION <IPlanAcquisition> : 
  - `[AcquisitionId <String>]`: Alım tanımlayıcısı.
  - `[AcquisitionTime <DateTime?>]`: Alım süresi.
  - `[ExternalReferenceId <String>]`: Dış başvuru tanımlayıcısı.
  - `[Id <String>]`: Kiracı aboneliği bağlamındaki tanımlayıcı.
  - `[PlanId <String>]`: Kiracı aboneliği bağlamında plan tanımlayıcısı.
