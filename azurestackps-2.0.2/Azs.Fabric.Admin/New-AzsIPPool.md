...
### -Numberofallocatedıaddress
Ayrılan IP adreslerinin sayısı.
...
### -Numberofıpadressesıntransition
Geçiş sırasında geçerli IP adresi sayısı.
...
### -Havuz
Bu kaynak, alt ağ içindeki düğümler için adreslerin ayrıldığı IP adresi aralığını tanımlar.
Oluşturmak için, havuz özellikleri için Notlar bölümüne bakın ve karma tablo oluşturun.
...
Havuz <IIPPool> : Bu kaynak, alt ağ içindeki düğümler için adreslerin AYRıLDıĞı IP adresi aralığını tanımlar.
  - `[Location <String>]`: Kaynağın bulunduğu bölge.
  - `[Tag <IResourceTags>]`: Anahtar-değer çiftleri listesi.
    - `[(Any) <String>]`: Bu, hiçbir özelliğin bu nesneye ekleneolabileceğini gösterir.
  - `[AddressPrefix <String>]`: Adres öneki.
  - `[EndIPAddress <String>]`: Bitiş IP adresi.
  - `[NumberOfAllocatedIPAddresses <Int64?>]`: Ayrılan IP adreslerinin sayısı.
  - `[NumberOfIPAddresses <Int64?>]`: IP adreslerinin toplam sayısı.
  - `[NumberOfIPAddressesInTransition <Int64?>]`: Geçiş sırasında geçerli IP adresi sayısı.
  - `[StartIPAddress <String>]`: Başlangıç IP adresi.
