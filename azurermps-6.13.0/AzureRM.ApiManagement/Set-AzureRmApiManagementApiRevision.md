...
## Tanım
**Set-Azurermapımanagementapirevision** cmdlet 'ı BIR Azure API yönetim API düzenlemesini değiştirir.
...
Cmdlet, `2` API 'nin düzeltmesini `echo-api` Yeni bir açıklama, protokol ve yol ile güncelleştirir.
...
### -Apırevision
Var olan API düzeltmesine ait tanımlayıcı. Bu parametre gereklidir.
...
Bu parametre isteğe bağlıdır.
Varsayılan değer $null.
, AuthorizationScope belirtilmişse belirtilmelidir.
...
### -InputObject
PsApiManagementApi örneği. Bu parametre gereklidir.
...
### -Ad
Web API adı.
Geliştirici ve yönetici portalları 'nda görüneceği şekliyle API 'nin genel adı.
Bu parametre gereklidir.
...
### -Geçiş
Eğer belirtilmişse, ayarlanmış API 'yi temsil eden Microsoft. Azure. Commands. Apsananad. ServiceManagement. model. Psapimanagementapı türü.
...
### -Yol
Web API yolu.
API 'nin Genel URL 'sinin son bölümü.
Bu URL, Web hizmetine istek göndermek için API tüketicileri tarafından kullanılacaktır.
...
### -Protokoller
Web API protokolleri (http, https).
API 'nın kullanılabileceği iletişim kuralları.
Bu parametre gereklidir.
Varsayılan değer $null.
...
### -ServiceUrl
API 'yi kullanan Web hizmetinin URL 'SI.
Bu URL yalnızca Azure API Yönetimi tarafından kullanılır ve genel kullanıma sunulacaktır.
1-2000 karakter uzunluğunda olmalıdır.
Bu parametre gereklidir.
...
Abonelik anahtarı üstbilgi adı.
Bu parametre isteğe bağlıdır.
Varsayılan değer $null.
...
[Get-Azurermapımanagementapirevision](./Get-AzureRmApiManagementApiRevision.md)

[New-Azurermapımanagementapirevision](./New-AzureRmApiManagementApiRevision.md)

[Remove-Azurermapımanagementapirevision](./Remove-AzureRmApiManagementApiRevision.md)
