...
## Tanım
**Set-AzureRmVMADDomainExtension** cmdlet 'ı bir Azure Active DIRECTORY (ad) etki alanı sanal makine uzantısını bir sanal makineye ekler.
Bu uzantı, sanal makinenizin bir etki alanına katılmasını sağlar.
...
### -JoinOption
Katılma seçeneğini belirtir. Birleştirme seçenekleri için bkz: [FJoinOptions](https://docs.microsoft.com/en-us/windows/desktop/api/lmjoin/nf-lmjoin-netjoindomain)
...
### -Ad
Eklenecek etki alanı uzantısının adını belirtir.
...
### -OUPath
Etki alanı hesabı için bir kuruluş birimi (OU) belirtir.
KURULUŞ biriminin tam ayırt edici adını tırnak içine girin.
Varsayılan değer, etki alanındaki makine nesnelerinin varsayılan OU 'dır.
...
### -Restart
Bu cmdlet 'in sanal makineyi yeniden başlattığını gösterir.
Değişikliğin etkili olması için genellikle yeniden başlatma gerekir.
...
### -TypeHandlerVersion
Etki alanı uzantısının sürümünü belirtir.
