---
Module Name: AzureRM.Resources
Module Guid: ab3ca893-26fe-44b0-bd3c-8933df144d7b
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/azurerm.resources
Help Version: 5.5.2.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/preview/src/ResourceManager/Resources/Commands.Resources/help/AzureRM.Resources.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/preview/src/ResourceManager/Resources/Commands.Resources/help/AzureRM.Resources.md
ms.openlocfilehash: e0fc92ebad2f169c80959e0ee1a5fe40360000cc
ms.sourcegitcommit: 43f4bdf2a59dd82fd881512aa9761bf72eb5703c
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 04/23/2019
ms.locfileid: "93570845"
---
# AzureRM. resources modülü
## Tanım
Bu konuda, Azure Resource Manager cmdlet 'Lerinin yardım konuları görüntülenir.

## AzureRM. kaynak cmdlet 'Leri
### [Add-AzureRmADGroupMember](Add-AzureRmADGroupMember.md)
Var olan bir AD grubuna kullanıcı ekler.

### [Dışarı aktarma-AzureRmResourceGroup](Export-AzureRmResourceGroup.md)
Kaynak grubu şablon olarak yakalar ve dosyaya kaydeder.

### [Get-AzureRmADAppCredential](Get-AzureRmADAppCredential.md)
Uygulamayla ilişkili kimlik bilgilerinin listesini alır.

### [Get-AzureRmADApplication](Get-AzureRmADApplication.md)
Var olan Azure Active Directory uygulamalarını listeler.

### [Get-AzureRmADGroup](Get-AzureRmADGroup.md)
Active Directory gruplarına filtre uygular.

### [Get-AzureRmADGroupMember](Get-AzureRmADGroupMember.md)
Geçerli Kiracıdaki bir AD grubunun üyelerini listeler.

### [Get-AzureRmADServicePrincipal](Get-AzureRmADServicePrincipal.md)
Active Directory hizmet sorumlularına filtre uygular.

### [Get-AzureRmADSpCredential](Get-AzureRmADSpCredential.md)
Hizmet sorumlusu ile ilişkili kimlik bilgilerinin listesini alır.

### [Get-AzureRmADUser](Get-AzureRmADUser.md)
Active Directory kullanıcılarına filtre uygular.

### [Get-AzureRmDeployment](Get-AzureRmDeployment.md)
Abonelik kapsamında bir dağıtım alır.

### [Get-AzureRmDeploymentOperation](Get-AzureRmDeploymentOperation.md)
Abonelik kapsamındaki bir dağıtımın dağıtım işlemlerini alır.

### [Get-AzureRmLocation](Get-AzureRmLocation.md)
Her konum için tüm konumları ve desteklenen kaynak sağlayıcılarını alır.

### [Get-AzureRmManagedApplication](Get-AzureRmManagedApplication.md)
Yönetilen uygulamaları alır

### [Get-AzureRmManagedApplicationDefinition](Get-AzureRmManagedApplicationDefinition.md)
Yönetilen uygulama tanımlarını alır

### [Get-AzureRmManagementGroup](Get-AzureRmManagementGroup.md)
Yönetim gruplarını alır

### [Get-AzureRmPolicyAssignment](Get-AzureRmPolicyAssignment.md)
İlke atamalarını alır.

### [Get-AzureRmPolicyDefinition](Get-AzureRmPolicyDefinition.md)
İlke tanımlarını alır.

### [Get-AzureRmPolicySetDefinition](Get-AzureRmPolicySetDefinition.md)
İlke kümesi tanımlarını alır.

### [Get-AzureRmProviderFeature](Get-AzureRmProviderFeature.md)
Azure sağlayıcı özellikleri hakkında bilgi alır.

### [Get-AzureRmProviderOperation](Get-AzureRmProviderOperation.md)
Azure RBAC kullanarak güvenliği sağlanabilir bir Azure Kaynak sağlayıcısı için işlemleri alır.

### [Get-AzureRmResource](Get-AzureRmResource.md)
Kaynakları alır.

### [Get-AzureRmResourceGroup](Get-AzureRmResourceGroup.md)
Kaynak gruplarını alır.

### [Get-AzureRmResourceGroupDeployment](Get-AzureRmResourceGroupDeployment.md)
Kaynak grubundaki dağıtımları alır.

### [Get-AzureRmResourceGroupDeploymentOperation](Get-AzureRmResourceGroupDeploymentOperation.md)
Kaynak grubu dağıtım işlemini alır

### [Get-AzureRmResourceLock](Get-AzureRmResourceLock.md)
Kaynak kilidini alır.

### [Get-AzureRmResourceProvider](Get-AzureRmResourceProvider.md)
Kaynak sağlayıcısını alır.

### [Get-Azurermroleödev](Get-AzureRmRoleAssignment.md)
Belirtilen kapsamda Azure RBAC rol atamalarını listeler.
Varsayılan olarak, seçilen Azure aboneliğindeki tüm rol atamalarını listeler.
Belirli bir kullanıcıya yönelik atamaları listelemek veya belirli bir kaynak grubundaki veya kaynağındaki atamaları listelemek için ilgili parametreleri kullanın.

### [Get-AzureRmRoleDefinition](Get-AzureRmRoleDefinition.md)
Ödev için kullanılabilen tüm Azure RBAC rollerini listeler.

### [Invoke-AzureRmResourceAction](Invoke-AzureRmResourceAction.md)
Kaynakta bir eylem çağırır.

### [Taşı-AzureRmResource](Move-AzureRmResource.md)
Kaynağı farklı bir kaynak grubuna veya aboneliğe taşıma.

### [Yeni-AzureRmADAppCredential](New-AzureRmADAppCredential.md)
Var olan bir uygulamaya kimlik bilgisi ekler.

### [Yeni-AzureRmADApplication](New-AzureRmADApplication.md)
Yeni bir Azure Active Directory uygulaması oluşturur.

### [Yeni-AzureRmADGroup](New-AzureRmADGroup.md)
Yeni bir Active Directory grubu oluşturur.

### [New-AzureRmADServicePrincipal](New-AzureRmADServicePrincipal.md)
Yeni bir Azure Active Directory hizmeti sorumlusu oluşturur.

### [Yeni-AzureRmADSpCredential](New-AzureRmADSpCredential.md)
Var olan hizmet sorumlusuna kimlik bilgileri ekler.

### [Yeni-AzureRmADUser](New-AzureRmADUser.md)
Yeni bir Active Directory kullanıcısı oluşturur.

### [Yeni-AzureRmDeployment](New-AzureRmDeployment.md)
Abonelik kapsamında yeni bir dağıtım oluşturur.

### [Yeni-AzureRmManagedApplication](New-AzureRmManagedApplication.md)
Azure yönetilen uygulaması oluşturur.

### [Yeni-AzureRmManagedApplicationDefinition](New-AzureRmManagedApplicationDefinition.md)
Yönetilen uygulama tanımı oluşturur.

### [Yeni-AzureRmManagementGroup](New-AzureRmManagementGroup.md)
Yönetim grubu oluşturur

### [Yeni-AzureRmManagementGroupSubscription](New-AzureRmManagementGroupSubscription.md)
Yönetim grubuna abonelik ekler.

### [New-AzureRmPolicyAssignment](New-AzureRmPolicyAssignment.md)
İlke ataması oluşturur.

### [New-AzureRmPolicyDefinition](New-AzureRmPolicyDefinition.md)
İlke tanımı oluşturur.

### [New-AzureRmPolicySetDefinition](New-AzureRmPolicySetDefinition.md)
İlke kümesi tanımı oluşturur.

### [Yeni-AzureRmResource](New-AzureRmResource.md)
Kaynak oluşturur.

### [Yeni-AzureRmResourceGroup](New-AzureRmResourceGroup.md)
Bir Azure Kaynak grubu oluşturur.

### [Yeni-AzureRmResourceGroupDeployment](New-AzureRmResourceGroupDeployment.md)
Kaynak grubuna Azure dağıtımı ekler.

### [New-AzureRmResourceLock](New-AzureRmResourceLock.md)
Kaynak kilidi oluşturur.

### [Yeni-Azurermroleataması](New-AzureRmRoleAssignment.md)
Belirtilen RBAC rolü, belirtilen kapsamda belirtilen sorumluya atar.

### [Yeni-AzureRmRoleDefinition](New-AzureRmRoleDefinition.md)
Azure RBAC 'de özel bir rol oluşturur.
Giriş olarak bir JSON rol tanımı dosyası veya PSRoleDefinition nesnesi sağlayın.
İlk olarak, temel rol tanımı nesnesi oluşturmak için Get-AzureRmRoleDefinition komutunu kullanın.
Ardından özelliklerini gerektiği gibi değiştirin.
Son olarak, rol tanımını kullanarak özel bir rol oluşturmak için bu komutu kullanın.

### [Register-AzureRmProviderFeature](Register-AzureRmProviderFeature.md)
Hesabınızda bir Azure sağlayıcı özelliği kaydeder.

### [Register-AzureRmResourceProvider](Register-AzureRmResourceProvider.md)
Kaynak sağlayıcısını kaydeder.

### [Remove-AzureRmADAppCredential](Remove-AzureRmADAppCredential.md)
Uygulamadaki kimlik bilgilerini kaldırır.

### [Remove-AzureRmADApplication](Remove-AzureRmADApplication.md)
Azure Active Directory uygulamasını siler.

### [Remove-AzureRmADGroup](Remove-AzureRmADGroup.md)
Active Directory grubunu siler.

### [Remove-AzureRmADGroupMember](Remove-AzureRmADGroupMember.md)
Bir AD grubundan kullanıcıyı kaldırır.

### [Remove-AzureRmADServicePrincipal](Remove-AzureRmADServicePrincipal.md)
Azure Active Directory hizmet sorumlusunu siler.

### [Remove-AzureRmADSpCredential](Remove-AzureRmADSpCredential.md)
Hizmet sorumlusunun kimlik bilgilerini kaldırır.

### [Remove-AzureRmADUser](Remove-AzureRmADUser.md)
Active Directory kullanıcısını siler.

### [Remove-AzureRmDeployment](Remove-AzureRmDeployment.md)
Aboneliği abonelik kapsamında kaldırır.

### [Remove-AzureRmManagedApplication](Remove-AzureRmManagedApplication.md)
Yönetilen uygulamayı kaldırır

### [Remove-AzureRmManagedApplicationDefinition](Remove-AzureRmManagedApplicationDefinition.md)
Yönetilen uygulama tanımını kaldırır

### [Remove-AzureRmManagementGroup](Remove-AzureRmManagementGroup.md)
Yönetim grubunu kaldırır

### [Remove-AzureRmManagementGroupSubscription](Remove-AzureRmManagementGroupSubscription.md)
Bir yönetim grubundan aboneliği kaldırır.

### [Remove-AzureRmPolicyAssignment](Remove-AzureRmPolicyAssignment.md)
İlke atamasını kaldırır.

### [Remove-AzureRmPolicyDefinition](Remove-AzureRmPolicyDefinition.md)
İlke tanımını kaldırır.

### [Remove-AzureRmPolicySetDefinition](Remove-AzureRmPolicySetDefinition.md)
İlke kümesi tanımını kaldırır.

### [Remove-AzureRmResource](Remove-AzureRmResource.md)
Kaynağı kaldırır.

### [Remove-AzureRmResourceGroup](Remove-AzureRmResourceGroup.md)
Kaynak grubunu kaldırır.

### [Remove-AzureRmResourceGroupDeployment](Remove-AzureRmResourceGroupDeployment.md)
Kaynak grubu dağıtımını ve ilişkili işlemleri kaldırır.

### [Remove-AzureRmResourceLock](Remove-AzureRmResourceLock.md)
Kaynak kilidini kaldırır.

### [Remove-Azurermroleataması](Remove-AzureRmRoleAssignment.md)
Belirli bir kapsamda belirli bir role atanmış olan belirtilen sorumluya rol atamasını kaldırır.

### [Remove-AzureRmRoleDefinition](Remove-AzureRmRoleDefinition.md)
Azure RBAC 'de özel bir rol siler.
Silinecek rol, rolün ID özelliği kullanılarak belirtilir.
Özel rolde yapılan rol atamaları varsa, silme işlemi başarısız olur.

### [Save-AzureRmDeploymentTemplate](Save-AzureRmDeploymentTemplate.md)
Abonelik kapsamındaki bir dağıtımın şablonunu alır.

### [Save-AzureRmResourceGroupDeploymentTemplate](Save-AzureRmResourceGroupDeploymentTemplate.md)
Bir dosyaya kaynak grubu dağıtım şablonunu kaydeder.

### [Set-AzureRmManagedApplication](Set-AzureRmManagedApplication.md)
Yönetilen uygulamayı güncelleştirir

### [Set-AzureRmManagedApplicationDefinition](Set-AzureRmManagedApplicationDefinition.md)
Yönetilen uygulama tanımını güncelleştirir

### [Set-AzureRmPolicyAssignment](Set-AzureRmPolicyAssignment.md)
Bir ilke atamasını değiştirir.

### [Set-AzureRmPolicyDefinition](Set-AzureRmPolicyDefinition.md)
İlke tanımını değiştirir.

### [Set-AzureRmPolicySetDefinition](Set-AzureRmPolicySetDefinition.md)
İlke kümesi tanımını değiştirme

### [Set-AzureRmResource](Set-AzureRmResource.md)
Bir kaynağı değiştirir.

### [Set-AzureRmResourceGroup](Set-AzureRmResourceGroup.md)
Kaynak grubunda değişiklik yapar.

### [Set-AzureRmResourceLock](Set-AzureRmResourceLock.md)
Kaynak kilidini değiştirir.

### [Set-AzureRmRoleDefinition](Set-AzureRmRoleDefinition.md)
Azure RBAC 'de özel bir rolü değiştirir.
Değiştirilmiş rol tanımını JSON dosyası veya PSRoleDefinition olarak sağlayın.
İlk olarak, değiştirmek istediğiniz özel rolü almak için Get-AzureRmRoleDefinition komutunu kullanın.
Ardından, değiştirmek istediğiniz özellikleri değiştirin.
Son olarak bu komutu kullanarak rol tanımını kaydedin.

### [Dur-AzureRmDeployment](Stop-AzureRmDeployment.md)
Abonelik kapsamında çalışan bir dağıtımı iptal eder.

### [Stop-AzureRmResourceGroupDeployment](Stop-AzureRmResourceGroupDeployment.md)
Kaynak grubu dağıtımını iptal eder.

### [Test-AzureRmDeployment](Test-AzureRmDeployment.md)
Abonelik kapsamındaki bir dağıtım şablonunu ve parametrelerini doğrular.

### [Test-AzureRmResourceGroupDeployment](Test-AzureRmResourceGroupDeployment.md)
Kaynak grubu dağıtımını doğrular.

### [Unregister-AzureRmResourceProvider](Unregister-AzureRmResourceProvider.md)
Kaynak sağlayıcının kaydını siler.

### [Güncelleştirme-AzureRmADApplication](Update-AzureRmADApplication.md)
Var olan bir Azure Active Directory uygulamasını güncelleştirir.

### [Güncelleştirme-AzureRmADServicePrincipal](Update-AzureRmADServicePrincipal.md)
Var olan bir Azure Active Directory hizmet sorumlusunu güncelleştirir.

### [Güncelleştirme-AzureRmADUser](Update-AzureRmADUser.md)
Var olan Active Directory kullanıcısını güncelleştirir.

### [Güncelleştirme-AzureRmManagementGroup](Update-AzureRmManagementGroup.md)
Yönetim grubunu güncelleştirir

