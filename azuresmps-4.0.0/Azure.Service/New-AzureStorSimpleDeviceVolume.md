...
## SYNOPSIS
Belirtilen birim kapsayıcısında birim oluşturur.
...
## Tanım
**New-AzureStorSimpleDeviceVolume** cmdlet 'i belirtilen birim kapsayıcısında bir birim oluşturur.
Bu cmdlet, her birimi bir veya daha fazla erişim denetimi kaydı ile ilişkilendirir.
**Accesscontrolrecord** nesnelerini almak için **Get-AzureStorSimpleAccessControlRecord** cmdlet 'ini kullanın.
Birim için bir ad, boyut ve AppType belirtin.
Ayrıca, varsayılan yedeklemenin etkinleştirilip etkinleştirilmeyeceğini ve izlemenin etkinleştirilip etkinleştirilmeyeceğini belirtin.

## ÖRNEKLERDEN

### Örnek 1: birim oluşturma
```
...
İlk komut, **Get-AzureStorSimpleAccessControlRecord** cmdlet 'Ini kullanarak StorSimple Manager hizmet yapılandırmasındaki erişim denetimi kayıtlarını alır ve bunları $AcrList değişkeninde depolar.

İkinci komut, **Get-AzureStorSimpleDeviceVolumeContainer** cmdlet 'ini kullanarak Contoso63-AppVm adlı cihaz için VolumeContainer07 adlı birim kapsayıcısını alır.
Komut, ardışık düzen işlecini kullanarak bu kapsayıcıyı geçerli cmdlet 'e geçirir.
Bu cmdlet bölümü oluşturur.
Komut, $AcrList depolanan birimin adını, boyutunu ve erişim denetimi kayıtlarını belirtir.
...
### Örnek 2: Access denetimleri olmadan birim oluşturma
```
...
Bu komut, **Get-AzureStorSimpleDeviceVolumeContainer** cmdlet 'ini kullanarak Contoso63-AppVm adlı cihaz için VolumeContainer01 adlı birim kapsayıcısını alır.
Komut, ardışık düzen işlecini kullanarak bu kapsayıcıyı geçerli cmdlet 'e geçirir.
Bu cmdlet bölümü oluşturur.
Komut, birimin adını, boyutunu ve Access denetim kayıtları için boş bir değeri belirtir.
Bu komut, *Waitforcomplete* parametresini belirtir, böylece birim oluşturulduktan sonra bir **taskstatusınfo** döndürür.

Komut hiçbir erişim denetimi kaydı içermiyorsa, bu birime erişilemez.
**Set-AzureStorSimpleDeviceVolume** cmdlet 'ini kullanarak erişim ekleyebilirsiniz.

## PARAMETRELERINE

### -AccessControlRecords
Birimle ilişkilendirilecek erişim denetimi kayıtlarının listesini belirtir.
...
### -Aygıtadı
Birimin oluşturulacağı StorSimple aygıtının adını belirtir.
...
### -EnableDefaultBackup
Birimde varsayılan yedeklemenin etkinleştirilip etkinleştirilmeyeceğini belirtir.
...
### -EnableMonitoring
Birimde izlemenin etkinleştirilip etkinleştirilmeyeceğini belirtir.
...
### -Çevrimiçi
Birimin çevrimiçi oluşturulup oluşturulmayacağını belirtir.
...
### -VolumeAppType
Birincil veya arşiv birimi oluşturulup oluşturulmayacağını belirtir.
Geçerli değerler: PrimaryVolume ve ArchiveVolume.
...
### -Birimkapsayıcısı
Birimi oluşturmak için bir **datacontainer** nesnesi olarak kapsayıcı belirtir.
**VirtualDisk** nesnesi edinmek için **Get-AzureStorSimpleDeviceVolumeContainer** cmdlet 'ini kullanın.
...
### -VolumeSizeInBytes
Bayt cinsinden birim boyutunu belirtir.
...
Bu cmdlet, bir **Datacontainer** nesnesini ve yeni birimin **accesscontrolrecord** nesnelerinin listesini kabul eder.
