...
## SYNOPSIS
Bir HDInsight işinin tamamlanmasını veya başarısızlığını bekler ve işin ilerlemesini görüntüler.
...
### HDInsight kümesinde JobId ile bekleme Işi
```
...
### HDInsight kümesinde Iş ile işi bekleme Işi
```
...
**Wait-AzureHDInsightJob** cmdlet 'ı bir Azure HDInsight işinin tamamlanması veya başarısızlığını bekler ve işin ilerlemesini görüntüler.

## ÖRNEKLERDEN

### Örnek 1: bir iş çalıştırıp tamamlamasını bekleyin
```
...
İlk komut geçerli Azure aboneliği KIMLIĞINI alır ve $SubId değişkeninde depolar.

İkinci komut belirtilen kümeyi alır ve $ClusterName değişkeninde depolar.
...
Dördüncü komut sırayla birkaç cmdlet kullanır: 

- **Başlangıç-AzureHDInsightJob** cmdlet 'ine $WordCountJob geçirmek için ardışık düzen işlecini kullanır. 
- İş, işin tamamlanması için 3600 saniye beklemek için **wait-AzureHDInsightJob** cmdlet 'ine geçirilir. 
- İş tamamlandığında, komut **Get-AzureHDInsightJobOutput** cmdlet 'ini kullanarak iş çıktısını alır.
...
### -Küme
Bir küme belirtir.
Bu cmdlet kümede bu parametrenin belirttiği işi bekler.
...
Bu parametreyi belirtmezseniz, varsayılan ad alanı kullanılır.
...
### -Job
Bir Azure HDInsight işi belirtir.
...
### -Abonelik
Aboneliği belirtir.
Bu cmdlet, bu parametrenin belirttiği abonelik için bir iş bekler.
...
### -Waittimeoutınseconds
Bekleme işlemi için zaman aşımını saniye cinsinden belirtir.
Zaman aşımı iş tamamlanmadan önce dolarsa, cmdlet çalışmaya sona erer.
