...
## <a name="510---november-2020"></a>5.1.0 - Kasım 2020
#### <a name="azaccounts"></a>Az.Accounts
* 'Connect-AzAccount -DeviceCode' kullanılıyorsa TenantId ile uyum sağlanamamasına neden olan bir sorun düzeltildi [#13477]
* Yeni 'Get-AzAccessToken' cmdlet’i eklendi
* Kullanıcı profili yoluna erişilemiyorsa hataya neden olan bir sorun düzeltildi
* Connect-AzAccount komutu sırasında Write-Object hatasına neden olan bir sorun düzeltildi [#13419]
* Şu cmdlet’lere 'ContainerRegistryEndpointSuffix' parametresi eklendi: 'Add-AzEnvironment', 'Set-AzEnvironment' 
* <kbd>CTRL</kbd>+<kbd>C</kbd> tuşlarına basıldığında oturum açma işleminin kesilmesi desteği eklendi
* 'Connect-AzAccount -KeyVaultAccessToken' komutunun çalışmamasına neden olan bir sorun düzeltildi [#13127]
* 'Invoke-AzRestMethod' komutundaki büyük/küçük harfe duyarlı olmayan boş referans ve yöntem düzeltildi

#### <a name="azaks"></a>Az.Aks
* Kullanıcının, yeni bir Kubernetes kümesi oluşturmak için hizmet sorumlusunu kullanamamasına neden olan bir sorun düzeltildi. [#13012]

#### <a name="azappconfiguration"></a>Az.AppConfiguration
* 'Az.AppConfiguration' modülü genel kullanıma sunuldu

#### <a name="azdatafactory"></a>Az.DataFactory
* 'New-AzDataFactoryV2LinkedServiceEncryptedCredential' komutunun hata iletisi iyileştirildi

#### <a name="azdatalakestore"></a>Az.DataLakeStore
* ADLS veri düzlemi SDK’sı 1.2.4-alpha sürümüne güncelleştirildi. Değişiklikler: https://github.com/Azure/azure-data-lake-store-net/blob/preview-alpha/CHANGELOG.md#version-124-alpha

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* Yeni MSIX Paketi cmdlet’leri eklendi ve Applications cmdlet’leri güncelleştirildi.

#### <a name="azeventhub"></a>Az.EventHub
* EventHub kümesine yönelik etiketi olmayan Küme komutları düzeltildi
* AzEventHubGeoDRConfiguration komutlarındaki PartnerNamespace için yardım metni güncelleştirildi 

#### <a name="azhdinsight"></a>Az.HDInsight
* Giden geçişi ve özel bağlantı özelliğini desteklemek için 'New-AzHDInsightCluster' cmdlet’ine 'ResourceProviderConnection' ve 'PrivateLink' parametreleri eklendi
* Özel Ambari veritabanı özelliğini desteklemek için 'New-AzHDInsightCluster' cmdlet’ine 'AmbariDatabase' parametresi eklendi
* 'Add-AzHDInsightMetastore' cmdlet’inin 'MetastoreType' parametresine 'AmbariDatabase' kabul değeri eklendi

#### <a name="aziothub"></a>Az.IotHub
* IoT Hub’ı oluşturma cmdlet’inde etiketlere izin verildi.

#### <a name="azkeyvault"></a>Az.KeyVault
* Anahtar kasası etiketini güncelleştirme desteği eklendi

#### <a name="azlogicapp"></a>Az.LogicApp
* Get-AzLogicAppRunHistory’nin sonuçların yalnızca ilk sayfasını getirmesine neden olan bir sorun düzeltildi

#### <a name="aznetwork"></a>Az.Network
* Aşağıdaki cmdlet güncelleştirildi 
    - 'New-AzLoadBalancerFrontendIpConfigCommand', 'Set-AzLoadBalancerFrontendIpConfigCommand', 'Add-AzLoadBalancerFrontendIpConfigCommand':
        - PublicIpAddressPrefix özelliği eklendi
        - PublicIpAddressPrefixId özelliği eklendi
* Genel yük dengelemeye olanak vermek için aşağıdaki cmdlet’lere yeni özellikler eklendi:
    - 'New-AzLoadBalancer':
        - SKU Tier özelliği eklendi
    - 'New-AzPuplicIpAddress':
        - SKU Tier özelliği eklendi
    - 'New-AzPublicIpPrefix':
        - SKU Tier özelliği eklendi
    - 'New-AzLoadBalancerBackendAddressConfig':
        - LoadBalancerFrontendIPConfigurationId özelliği eklendi
* Şu cmdlet’ler için kullanımdan kaldırma planlamasına yönelik uyarılar güncelleştirildi:   -'New-AzVirtualHubRoute'   -'New-AzVirtualHubRouteTable'   -'Add-AzVirtualHubRoute'   -'Add-AzVirtualHubRouteTable'   -'Get-AzVirtualHubRouteTable'   -'Remove-AzVirtualHubRouteTable'
* Şu cmdlet’ler için 'RouteTable' bağımsız değişkeninin kullanımdan kaldırma planlamasına yönelik uyarılar eklendi:   -'New-AzVirtualHub'   -'Set-AzVirtualHub'   -'Update-AzVirtualHub'
* '-MinScaleUnits' ve '-MaxScaleUnits' bağımsız değişkenleri, 'Set-AzExpressRouteGateway' içinde isteğe bağlı hale getirildi
* Application Gateway’de Karşılıklı Kimlik Doğrulama ve SSL Profilleri’ni desteklemek için yeni cmdlet’ler eklendi
...
* İlke BackupTime değeri UTC olarak belirtilir.
* Get-AzRecoveryServicesBackupJobDetails cmdlet’inde hataya neden olan değişiklik uyarısı değiştirildi.
* Set-AzRecoveryServicesBackupProtectionPolicy cmdlet’i için örnek betik yardım metni güncelleştirildi.

#### <a name="azresources"></a>Az.Resources
* Etki değerlendirmesinin farklı büyük/küçük harf düzenlerine sahip iki kaynak grubu kapsamı göstermesine neden olan bir hata düzeltildi
* 'Export-AzResourceGroup' SDK’yı kullanacak biçimde güncelleştirildi.
* Ayrıştırma yöntemlerine kültür bilgileri eklendi

#### <a name="azsql"></a>Az.Sql
* Set-AzSqlDatabaseAudit’in Hiper Ölçek veritabanında desteklenmemesine ve veritabanı sürümünün belirlenememesine neden olan sorunlar düzeltildi
* 'New-AzSqlInstance' ve 'Set-AzSqlInstance' için MaintenanceConfigurationId eklendi
* GetAzureSqlDatabaseReplicationLink.cs içinde PartnerServerName parametresinin anahtar yerine değere göre denetlenmesine neden olan bir hata düzeltildi

#### <a name="azwebsites"></a>Az.Websites
* Yeni erişim kısıtlama özellikleri için destek eklendi: ServiceTag, multi-ip ve http-headers

### <a name="thanks-to-our-community-contributors"></a>Topluluğumuzdan katkıda bulunanlara teşekkür ederiz
* John Q. Martin (@johnmart82), Güvenlik duvarı önkoşul bilgilerinin eklenmesi (#13385)
* Manikandan Duraisamy (@madurais-msft), PublicSubnetName bağımsız değişkeninin düzeltilmesi (#13417)
* @mahortas, -HostNames parametre değerlerine yönelik güncelleştirme (#13349)
* @MariachiForHire, desteklenen TrafficAnalyticsInterval değerlerinin eklenmesi (#13304)
* Michael James (@mikejwhat), Get-AzLogicAppRunHistory komutunun 30’dan fazla giriş döndürmesinin sağlanması (#13393)
* Shashikant Shakya (@shshakya), Restore-AzSqlInstanceDatabase.md güncelleştirmesi (#13404)

## <a name="500---october-2020"></a>5.0.0 - Ekim 2020
#### <a name="azaccounts"></a>Az.Accounts
* [Yeni Değişiklik] 'Get-AzProfile' ve 'Select-AzProfile' kaldırıldı
* Azure Directory Authentication Library, Microsoft Authentication Library (MSAL) ile değiştirildi

#### <a name="azaks"></a>Az.Aks
* [Yeni Değişiklik] 'New-AzAksCluster' ve 'Set-AzAksCluster' cmdlet’indeki 'ClientIdAndSecret' parametre diğer adı kaldırıldı.
* [Yeni Değişiklik] 'New-AzAksCluster' cmdlet’indeki 'NodeVmSetType' parametresinin 'AvailabilitySet' varsayılan değeri 'VirtualMachineScaleSets' olarak değiştirildi.
* [Yeni Değişiklik] 'New-AzAksCluster' cmdlet’indeki 'NetworkPlugin' parametresinin 'None' varsayılan değeri 'azure' olarak değiştirildi.
* [Yeni Değişiklik] Linux’ta yalnızca bir değeri desteklediğinden 'New-AzAksCluster' cmdlet’indeki 'NodeOsType' parametresi kaldırıldı.

#### <a name="azbilling"></a>Az.Billing
* 'Get-AzBillingAccount' cmdlet’i eklendi
* 'Get-AzBillingProfile' cmdlet’i eklendi
* 'Get-AzInvoiceSection' cmdlet’i eklendi
* 'Get-AzBillingInvoice' cmdlet’ine yeni parametreler eklendi
* Get-AzBillingInvoice cmdlet’inin yanıtından DownloadUrlExpiry, Type ve BillingPeriodNames özellikleri kaldırıldı

#### <a name="azcdn"></a>Az.Cdn
* Çoklu kaynak ve özel bağlantı işlevini destekleyecek cmdlet’ler eklendi 

#### <a name="azcognitiveservices"></a>Az.CognitiveServices
* SDK sürümü 7.4.0-preview olarak güncelleştirildi.

#### <a name="azcompute"></a>Az.Compute
* 'New-AzVm' cmdlet’ine '-VmssId' parametresi eklendi
* 'New-AzVmss' cmdlet’ine 'PlatformFaultDomainCount' parametresi eklendi.
* Yeni 'Get-AzDiskEncryptionSetAssociatedResource' cmdlet’i
* New-AzDiskConfig cmdlet’ine isteğe bağlı 'Tier' ve 'LogicalSectorSize' parametreleri eklendi. 
...
* `-EncryptionKeyVersion` başarısız olmadan Databricks çalışma alanını güncelleştirmeye neden olabilecek bir hata düzeltildi.

#### <a name="azdatafactory"></a>Az.DataFactory
* ADF .NET SDK’sı 4.12.0 sürümüne güncelleştirildi
* ADF şifrelemesi istemci SDK’sı 4.14.7587.7 sürümüne güncelleştirildi
* 'Stop-AzDataFactoryV2TriggerRun' ve 'Invoke-AzDataFactoryV2TriggerRun' komutları eklendi

#### <a name="azdesktopvirtualization"></a>Az.DesktopVirtualization
* Üst düzey ARM nesneleri oluşturmak için Location özelliği gerekli kılındı.
        * `New-AzWvdApplicationGroup` için `ApplicationGroupType` gerekli kılındı.
        * `New-AzWvdApplicationGroup` için `HostPoolArmPath` gerekli kılındı.
        * `New-AzWvdHostPool` cmdlet’ine `PreferredAppGroupType` eklendi.

#### <a name="azfunctions"></a>Az.Functions
* [Yeni Değişiklik] 'IncludeSlot' anahtar parametresi, 'Get-AzFunctionApp' cmdlet’inin parametre kümelerinden biri hariç tümünden kaldırıldı. Cmdlet, şimdi '-IncludeSlot' belirtildiğinde sonuçlarda dağıtım yuvalarının alınmasını destekliyor. 
* 'New-AzFunctionApp' güncelleştirildi:
  - -DisableApplicationInsights, bu seçenek belirtildiğinde hiçbir Application Insights projesi oluşturulmayacak şekilde düzeltildi. [#12728]
  - [Yeni Değişiklik] PowerShell 6.2 işlev uygulamaları oluşturma desteği kaldırıldı.
  - [Yeni Değişiklik] RuntimeVersion parametresi belirtilmediğinde, PowerShell işlev uygulamaları için Windows üzerinde İşlevler sürüm 3’teki varsayılan çalışma zamanı 6.2 sürümünden 7.0 sürümüne yükseltildi.
  - [Yeni Değişiklik] RuntimeVersion parametresi belirtilmediğinde, Node işlev uygulamaları için Windows ve Linux üzerinde sürüm 3’teki varsayılan çalışma zamanı 10 sürümünden 12 sürümüne yükseltildi.
  - [Yeni Değişiklik] RuntimeVersion parametresi belirtilmediğinde, Python işlev uygulamaları için Linux üzerinde sürüm 3’teki varsayılan çalışma zamanı 3.7 sürümünden 3.8 sürümüne yükseltildi.

#### <a name="azhdinsight"></a>Az.HDInsight
 * New-AzHDInsightCluster cmdlet’inde:
     - 'DefaultStorageAccountName' parametresi 'StorageAccountResourceId' ile değiştirildi
     - 'DefaultStorageAccountKey' parametresi 'StorageAccountKey' ile değiştirildi
     - 'DefaultStorageAccountType' parametresi 'StorageAccountType' ile değiştirildi
     - 'PublicNetworkAccessType' parametresi kaldırıldı
     - 'OutboundPublicNetworkAccessType' parametresi kaldırıldı
     - Yeni parametreler eklendi: ADLS 2. Nesil’i desteklemek üzere 'StorageFileSystem' ve 'StorageAccountManagedIdentity' eklendi
     - HDInsight Kimlik Aracısını desteklemek üzere yeni 'EnableIDBroker' parametresi eklendi
     - Yeni parametreler eklendi: Kafka REST Proxy’yi desteklemek üzere 'KafkaClientGroupId', 'KafkaClientGroupName' ve 'KafkaManagementNodeSize' eklendi
 * New-AzHDInsightClusterConfig cmdlet’inde:
     - 'DefaultStorageAccountName' parametresi 'StorageAccountResourceId' ile değiştirildi
     - 'DefaultStorageAccountKey' parametresi 'StorageAccountKey' ile değiştirildi
     - 'DefaultStorageAccountType' parametresi 'StorageAccountType' ile değiştirildi
     - 'PublicNetworkAccessType' parametresi kaldırıldı
     - 'OutboundPublicNetworkAccessType' parametresi kaldırıldı
* Set-AzHDInsightDefaultStorage cmdlet’inde:
    - 'StorageAccountName' parametresi 'StorageAccountResourceId' ile değiştirildi
* Add-AzHDInsightSecurityProfile cmdlet’inde:
    - 'Domain' parametresi 'DomainResourceId' ile değiştirildi
    - 'OrganizationalUnitDN' parametresi için zorunlu gereksinim kaldırıldı

#### <a name="azkeyvault"></a>Az.KeyVault
* [Yeni Değişiklik] 'New-AzKeyVault' cmdlet’indeki DisableSoftDelete ve 'Update-AzKeyVault' cmdlet’indeki EnableSoftDelete parametreleri kullanımdan kaldırıldı
* [Yeni Değişiklik] Doğrudan SecretValue değerinin görüntülenmesini önlemek amacıyla SecretValueText özniteliği kaldırıldı [#12266]
* Şu yeni kaynak türü için destek eklendi: yönetilen HSM
    - Yönetilen HSM’deki anahtarları çalıştırmaya yönelik, yönetilen HSM ve cmdlet’lerin CRUD’si
    - Tam HSM yedeklemesi/geri yüklemesi, AES anahtarı oluşturma, güvenlik etki alanı yedeklemesi/geri yüklemesi, RBAC

#### <a name="azmanagedservices"></a>Az.ManagedServices
* [Yeni Değişiklik] Parametrelerin adlandırma kuralları ve ilişkili örnekleri güncelleştirildi

#### <a name="aznetwork"></a>Az.Network
* [Yeni Değişiklik] 'HostedSubnet' parametresi kaldırıldı ve yerine 'Subnet' eklendi
* Sanal Yönlendirici Eş Düğüm Yolları için yeni cmdlet’ler eklendi
    - 'Get-AzVirtualRouterPeerLearnedRoute'
    - 'Get-AzVirtualRouterPeerAdvertisedRoute'
* New-AzFirewall cmdlet'i güncelleştirildi:
    - '-SkuTier' parametresi eklendi
    - '-SkuName' parametresi eklendi ve bu parametreye yönelik SKU, Diğer Ad haline getirildi
    - '-Sku' parametresi kaldırıldı
* [Yeni Değişiklik] 'Start-AzVpnConnectionPacketCapture' ve 'Stop-AzVpnConnectionPacketCapture' cmdlet’lerinde 'Connectionlink' bağımsız değişkeni zorunlu kılındı
* [Yeni Değişiklik] 'New-AzNetworkWatcherConnectionMonitorEndPointObject', '-Filter' parametresi kaldırılacak şekilde güncelleştirildi
* [Yeni Değişiklik] 'New-AzNetworkWatcherConnectionMonitorEndpointFilterItemObject' cmdlet’i 'New-AzNetworkWatcherConnectionMonitorEndpointScopeItemObject' ile değiştirildi
* 'New-AzNetworkWatcherConnectionMonitorEndPointObject' cmdlet’i güncelleştirildi:
    - '-Type' parametresi eklendi
    - '-CoverageLevel' parametresi eklendi
    - '-Scope' parametresi eklendi
* 'New-AzNetworkWatcherConnectionMonitorProtocolConfigurationObject' cmdlet’i yeni '-DestinationPortBehavior' parametresiyle güncelleştirildi

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* Katkıda bulunan izinleri için İş Yükü Geri Yüklemesi düzeltildi.
* Restore-AzRecoveryServicesBackupItem cmdlet’ine yeni parametre kümeleri ve doğrulamalar eklendi.

#### <a name="azresources"></a>Az.Resources
* Ayrıştırma hataları düzeltildi
* ARM şablonu What-If cmdlet’leri, sonuçlardan önizleme iletisi kaldırılacak şekilde güncelleştirildi
* '-WhatIf' cmdlet’i daha yüksek bir kapsama ayarlandığında, şablon dağıtım cmdlet’lerinin kilitlenmesiyle ilgili bir sorun düzeltildi [#13038]
* Şablon dağıtım cmdlet’lerinin şablon parametrelerindeki büyük/küçük harfi korumamasıyla ilgili bir sorun düzeltildi
* 'Export-AzResourceGroup' cmdlet’inde kullanılmak üzere varsayılan API sürümü eklendi
* Şablon Belirtimlerine yönelik cmdlet’ler ('Get-AzTemplateSpec', 'Set-AzTemplateSpec', 'New-AzTemplateSpec', 'Remove-AzTemplateSpec', 'Export-AzTemplateSpec') eklendi
* Mevcut dağıtım cmdlet’lerini kullanarak (yeni -TemplateSpecId parametresi aracılığıyla) Şablon Belirtimlerini dağıtma desteği eklendi 
* 'Get-AzResourceGroupDeploymentOperation', SDK’yı kullanacak şekilde güncelleştirildi.
...
* Eski RestorePolicy.LastEnabledTime özelliği kaldırıldı
    - 'Enable-AzStorageBlobRestorePolicy'
    - 'Disable-AzStorageBlobRestorePolicy'
    - 'Get-AzStorageBlobServiceProperty'
    - “Update-AzStorageBlobServiceProperty”
* DaysAfterModificationGreaterThan parametresinin türü int yerine int? olarak değiştirildi
    - 'Set-AzStorageAccountManagementPolicy'
    - 'Get-AzStorageAccountManagementPolicy'
    - 'Add-AzStorageAccountManagementPolicyAction'
    - 'New-AzStorageAccountManagementPolicyRule'
* Erişim katmanına sahip dosya paylaşımı oluşturma/güncelleştirme desteği eklendi
    - 'New-AzRmStorageShare'
    - 'Update-AzRmStorageShare'
* Data Lake 2. Nesil’deki ACL’yi özyinelemeli olarak ayarlama/güncelleştirme/kaldırma desteği eklendi 
    -  'Set-AzDataLakeGen2AclRecursive' 
    -  'Update-AzDataLakeGen2AclRecursive' 
    -  'Remove-AzDataLakeGen2AclRecursive'
* Yeni x,t iznine sahip Kapsayıcı erişim ilkesi desteği eklendi
    -  'New-AzStorageContainerStoredAccessPolicy'
    -  'Set-AzStorageContainerStoredAccessPolicy'
* Permission alt özelliğinin türü enum’dan String’e değiştirilerek Kapsayıcı erişim ilkesi cmdlet’ini alma/ayarlama işleminin çıkışı değiştirildi
    -  'Get-AzStorageContainerStoredAccessPolicy'
    -  'Set-AzStorageContainerStoredAccessPolicy'
* JSON ile yönetim ilkesi ayarlamayla ilgili örnek betik sorunu düzeltildi
    -  'Set-AzStorageAccountManagementPolicy'

#### <a name="azwebsites"></a>Az.Websites
* Premium V3 fiyatlandırma katmanı desteği eklendi
* WebSites SDK 3.1.0 sürümüne güncelleştirildi

### <a name="thanks-to-our-community-contributors"></a>Topluluğumuzdan katkıda bulunanlara teşekkür ederiz
* @atul-ram, Get-AzDelegation.md güncelleştirildi (#13176)
* @dineshreddy007, Stack HCI kaydının WAC belirtecini kullanması durumunda doğru atanan Uygulama Rolleri alınıyor. (#13249)
* @kongou-ae, New-AzOffice365PolicyProperty.md güncelleştirildi (#13217)
* Lohith Chowdary Chilukuri (@Lochiluk), Set-AzApplicationGateway.md güncelleştirildi (#13150)
* Matthew Burleigh (@mburleigh)
  * Belgede başvurulan PowerShell cmdlet’lerine bağlantılar eklendi (#13203)
  * Belgede başvurulan PowerShell cmdlet’lerine bağlantılar eklendi (#13190)
  * Belgede başvurulan PowerShell cmdlet’lerine bağlantılar eklendi (#13189)
  * Başvurulan cmdlet’lere bağlantılar eklendi (#13137)
  * Belgede başvurulan PowerShell cmdlet’lerine bağlantılar eklendi (#13204)
  * Belgede başvurulan PowerShell cmdlet’lerine bağlantılar eklendi (#13205)

## <a name="480---october-2020"></a>4.8.0 - Ekim 2020
#### <a name="azaccounts"></a>Az.Accounts
* Ortak kitaplıklardaki DateTime ayrıştırma sorunu düzeltildi [#13045]

#### <a name="azcognitiveservices"></a>Az.CognitiveServices
* 'New-AzCognitiveServicesAccountApiProperty' cmdlet'i eklendi.
* 'New-AzCognitiveServicesAccount' ve 'Set-AzCognitiveServicesAccount' için 'ApiProperty' parametresi destekleniyor

#### <a name="azcompute"></a>Az.Compute
* 'Update-ASRRecoveryPlan' cmdlet'indeki sorun Yük Devretme Türleri doldurularak düzeltildi
* 'Get-AzVmImage' cmdlet'ine isteğe bağlı '-Top' ve '-OrderBy' parametreleri eklendi. 

#### <a name="azdatabricks"></a>Az.Databricks
* 'Az.Databricks' modülü genel kullanıma sunuldu
* Sanal ağ eşlemesi için destek eklendi

#### <a name="azdatafactory"></a>Az.DataFactory
* Çıkış iletilerindeki yazım hatası düzeltildi

#### <a name="azeventhub"></a>Az.EventHub
* 'Set-AzEventHubNetworkRuleSet' cmdlet'ine isteğe bağlı 'TrustedServiceAccessEnabled' anahtar parametresi eklendi

#### <a name="azhdinsight"></a>Az.HDInsight
* 'PublicNetworkAccessType' ve 'OutboundPublicNetworkAccessType' parametrelerini kullanımdan kaldırma planlaması için uyarı iletisi eklendi
* 'DefaultStorageAccountName' parametresini 'StorageAccountResourceId' ile değiştirme planlaması için uyarı iletisi eklendi
* 'DefaultStorageAccountKey' parametresini 'StorageAccountKey' ile değiştirme planlaması için uyarı iletisi eklendi
* 'DefaultStorageAccountType' parametresini 'StorageAccountType' ile değiştirme planlaması için uyarı iletisi eklendi
* 'DefaultStorageContainer' parametresini 'StorageContainer' ile değiştirme planlaması için uyarı iletisi eklendi
* 'DefaultStorageRootPath' parametresini 'StorageRootPath' ile değiştirme planlaması için uyarı iletisi eklendi

#### <a name="aziothub"></a>Az.IotHub
* Cihazların sdk'sı güncelleştirildi.

#### <a name="azkeyvault"></a>Az.KeyVault
* SecretValueText özelliğinin kaldırılacağı ayrıntılı tarih sağlandı

#### <a name="azmanagedservices"></a>Az.ManagedServices
* Yönetilen hizmet atama ve tanım cmdlet’lerine hataya neden olan değişiklik uyarıları güncelleştirildi

#### <a name="azmonitor"></a>Az.Monitor
* Uyarı iletisinin gizlenememesi hatası düzeltildi. [#12889]
* Uyarı kuralı ölçütlerinde 'SkipMetricValidation' parametresi destekleniyor. Ölçüm doğrulamasının atlanmasına neden olarak henüz yayılmamış özel bir ölçümle ilgili uyarı kuralı oluşturmaya olanak sağlar.

#### <a name="aznetwork"></a>Az.Network
* VPNSite Kaynağına Office365 İlkesi eklendi
    - 'New-AzO365PolicyProperty'

#### <a name="azrecoveryservices"></a>Az.RecoveryServices
* İş yükü yedeklemesi için kapsayıcı adı doğrulaması eklendi.

#### <a name="azrediscache"></a>Az.RedisCache
* Microsoft.Cache RP kaydıyla ilgili izin sorunundan dolayı 'New-AzRedisCache' ve 'Set-AzRedisCache' cmdlet'lerinin başarısız olmaması sağlandı

#### <a name="azsql"></a>Az.Sql
* Aşağıdakilere BackupStorageRedundancy eklendi: 
    - 'Restore-AzureRmSqlDatabase'
    - 'New-AzSqlDatabaseCopy'
    - 'New-AzSqlDatabaseSecondary'
* Tüm SQL DB başvurularında BackupStorageRedundancy parametresi için büyük/küçük harf duyarlılığı kaldırıldı 
* BackupStorageRedundancy uyarı iletisi adları güncelleştirildi

#### <a name="azstorage"></a>Az.Storage
* Depolama hesabının dosya hizmetinde paylaşımı etkinleştirme/devre dışı bırakma/alma geçici silme özellikleri destekleniyor
    - 'Update-AzStorageFileServiceProperty'
    - 'Get-AzStorageFileServiceProperty'
* Desteklenen liste dosya paylaşımları Depolama hesabının silinmiş olan dosyalarını içeriyor ve Tek dosya paylaşımı kullanımı alınıyor
    - 'Get-AzRmStorageShare'
* Silinmiş dosya paylaşımını geri yükleme destekleniyor
    - 'Restore-AzRmStorageShare'
* Blob hizmeti özelliklerini değiştirmeye yönelik cmdlet'ler değiştirildi. Sunucudan ilk özellikler alınmayacak, yalnızca değiştirilen özellikler sunucuya ayarlanacak.
    - 'Enable-AzStorageBlobDeleteRetentionPolicy'
    - 'Disable-AzStorageBlobDeleteRetentionPolicy'  
    - 'Enable-AzStorageBlobRestorePolicy'
    - 'Disable-AzStorageBlobRestorePolicy'
    - “Update-AzStorageBlobServiceProperty”
* New-AzStorageAccount parametresi -Kind varsayılan değeri için yardım sorunu düzeltildi [#12189]
* Blob karşıya yüklemesinde doğru ContentType ayarlama işlemini göstermek için örnek eklenerek sorun düzeltildi [#12989]
...
* Önceden hep '' olan 'ApplicationPackages' özelliği 'PSApplication' öğesinden kaldırıldı.
  - Uygulamanın içindeki belirli paketler artık 'Get-AzBatchApplicationPackage' kullanılarak alınabilir. Örnek: 'Get-AzBatchApplication -AccountName hesabım -ResourceGroupName kaynakgrubum -ApplicationId uygulamam'.
...
* **New-AzApiManagementProduct** _ ve _ *Set-AzApiManagementProduct**
