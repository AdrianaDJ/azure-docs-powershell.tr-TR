...
---
# <a name="persist-azure-user-credentials-across-powershell-sessions"></a>Azure kullanıcı kimlik bilgilerini PowerShell oturumlarında kalıcı hale getirme
...
- [Add-AzAccount][login] - İşlem veya geçerli kullanıcı için oturum açma kapsamının ayarlanmasına izin verir.
  Kimlik doğrulamasından sonra varsayılan bağlamı adlandırmaya olanak tanır.
- [Import-AzContext][import] - İşlem veya geçerli kullanıcı için oturum açma kapsamının ayarlanmasına izin verir.
- [Set-AzContext][set-context] - Mevcut adlandırılmış bağlamların ve işlem ya da geçerli kullanıcı kapsam değişikliklerinin seçilmesine olanak tanır.
