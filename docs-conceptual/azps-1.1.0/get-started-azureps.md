...
`Connect-AzAccount` cmdlet'iyle etkileşimli olarak oturum açın. Cloud Shell kullanıyorsanız bu adımı atlayın: Auzre Cloud Shell oturumunuz, Cloud Shell oturumunu başlatan ortam, abonelik ve kiracı için zaten kimlik doğrulaması yapmıştır.
...
https://microsoft.com/devicelogin sayfasında kullanabileceğiniz bir belirteç elde edersiniz. Tarayıcınızda bu sayfayı açıp belirteci girin, ardından Azure hesabı kimlik bilgilerinizle oturum açın ve Azure PowerShell’i yetkilendirin. 
