...
### Örnek 1: özel bir sağlayıcıyı kaldırma.
```powershell
PS C:\> PS C:\> Remove-AzCustomProvider -ResourceGroupName myRg -Name Namespace.Type
```

Özel sağlayıcıyı kaldırma

### Örnek 2: geçiş ile özel bir sağlayıcıyı kaldırma
```powershell
...
Başarı veya başarısızlığı göstermek için geçiş özelliğini kullanarak özel bir sağlayıcıyı kaldırın.
