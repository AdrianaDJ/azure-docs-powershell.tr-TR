...
## Tanım
**New-AzFrontDoorBackendpoolsSettingObject** cmdlet 'ı ön kapı oluşturmak için yeni bir PSBackendPoolsSettings nesnesi oluşturur.

## ÖRNEKLERDEN

### Örnek 1: Varsayılanları kullanarak BackendPoolsSettings nesnesini oluşturma
```powershell
...
### Örnek 2: kullanıcının belirttiği değerleri içeren BackendPoolsSettings nesnesi oluşturma
```powershell
...
### -Enforcecercertificate Atenamecheck
HTTPS isteklerinin tüm arka uç havuzlarına sertifika adı denetimini zorunlu tutma.
HTTPS olmayan isteklerde hiçbir etkiye gerek yoktur.
...
### -Sendrecvtimeoutınseconds
İsteğin arka uca iletilmesi sırasında zaman aşımı gönderme ve alma. Zaman aşımına ulaşıldığında, istek başarısız olur ve geri döner.
...
### Microsoft. Azure. Commands. Frontkapısı. modeller. PSBackendPoolsSetting
