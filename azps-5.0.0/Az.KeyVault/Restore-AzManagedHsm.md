...
## SYNOPSIS
Yönetilen bir HSM yedeklemeden tümüyle geri yükler.
...
## Tanım
Yönetilen bir HSM 'yi depolama hesabında depolanan bir yedekten tümüyle geri yükler.
`Backup-AzManagedHsm`Yedekleme için kullanın.
...
Örnek, "https://{accountName}. blob. Core. Windows. net/{Kapsayıcıadı}" depolama kapsayıcısının "mhsm-myHsm-2020101308504935" adlı klasörde depolanan bir yedeklemeyi geri yükler.

## PARAMETRELERINE

### -BackupFolder
Yedeğin klasör adı, örneğin ' mhsm- *-2020101309020403 '. Ayrıca, ' yedekler/mhsm--2020101309020403 ' gibi iç içe geçmiş olabilir*.
...
### -Geçiş
HSM geri yüklendiğinde doğru döner.
...
### -SasToken
Depolama hesabının kimliğini doğrulamak için paylaşılan erişim imzası (SAS) simgesi.
...
### -StorageAccountName
Yedeklemenin depolanacağı depolama hesabının adı.
...
Yedeklemenin depolanacağı blob kapsayıcısının adı.
...
### -StorageContainerUri
Yedeklemenin depolanacağı depolama kapsayıcısının URI 'SI.
