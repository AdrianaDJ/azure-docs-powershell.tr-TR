...
## SYNOPSIS
Yönetilen bir HSM 'yi tümüyle yedekleme.
...
## Tanım
Yönetilen bir HSM 'yi depolama hesabına tam olarak yedekleme.
`Restore-AzManagedHsm`Yedeği geri yüklemek için kullanın.
...
Cmdlet, depolama kapsayıcısında bir klasör (genellikle adı `mhsm-{name}-{timestamp}` ) oluşturur, yedeği bu klasörde depolar ve klasör URI 'sini çıktısını alır.
