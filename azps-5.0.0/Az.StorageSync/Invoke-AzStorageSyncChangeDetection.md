...
> [!Note]  
> Invoke-AzStorageSyncChangeDetection cmdlet 'i, Azure dosya paylaşımında aşağıdaki değişiklikleri algılamaz:
> - Silinen dosyalar. 
> - Paylaşımdan taşınan dosyalar.
> - Aynı adla silinen ve oluşturulan dosyalar.  
> 
>  [Değişiklik algılama işi](https://docs.microsoft.com/azure/storage/files/storage-sync-files-troubleshoot?tabs=portal1%2Cazure-portal#afs->change-detection) çalıştığında bu değişiklikler saptanacaktır.
...
## Tanım
Azure dosya eşitlemesi belirli aralıklarla, dosya paylaşımının eşitleme içindeki ad alanını, eşitlenenden başka yollarla denetler. Amaç, bu değişiklikleri ve son olarak bağlı sunuculara eşitlemeyi tanımlamaktır. Bu komut, ad alanları değişikliklerinin algılanmasını el ile başlatmak için kullanılabilir. Tüm paylaşım, alt klasör veya dosya kümesini hedeflenebilir. Değişiklik kapsamı sizin için biliniyorsa, bu komutun yürütülmesini ad alanının bölümleriyle sınırlandırın, böylece algılama özelliğinin hızla ve 10.000 öğeleri sınırının içinde tamamlanabilmesi için
...
Bu örnekte, "örnekler" dizininde algılama algılaması çalıştırılır ve alt dizinlerdeki değişiklikleri yinelemeli olarak algılar. Yol 10.000 ' den fazla öğe içeriyorsa cmdlet 'in başarısız olacağını unutmayın. Yol 10.000 ' den fazla öğe içeriyorsa, bu komutu ad alanının alt kısımlarında çalıştırabilirsiniz.
...
### -Ad
Cloudenvseçpunto adı. Ad, portalda görüntülenen kolay adın değil, bir GUID. Cloudenvseçpointname 'i almak için Get-AzStorageSyncCloudEndpoint cmdlet 'ini kullanın.
