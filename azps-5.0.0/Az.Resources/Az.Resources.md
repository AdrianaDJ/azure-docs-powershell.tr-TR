---
Module Name: Az.Resources
Module Guid: ab3ca893-26fe-44b0-bd3c-8933df144d7b
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.resources
Help Version: 5.5.2.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Resources/Resources/help/Az.Resources.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Resources/Resources/help/Az.Resources.md
ms.openlocfilehash: 25460ff58e2c3f2a493638cc7011e352d551280e
ms.sourcegitcommit: b4a38bcb0501a9016a4998efd377aa75d3ef9ce8
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 10/27/2020
ms.locfileid: "94322423"
---
# Az. resources modülü
## Tanım
Bu konuda, Azure Resource Manager cmdlet 'Lerinin yardım konuları görüntülenir.

## Az. kaynak cmdlet 'Leri
### [Add-AzADGroupMember](Add-AzADGroupMember.md)
Var olan bir AD grubuna kullanıcı ekler.

### [Dışarı aktarma-AzResourceGroup](Export-AzResourceGroup.md)
Kaynak grubu şablon olarak yakalar ve dosyaya kaydeder.

### [Get-AzADAppCredential](Get-AzADAppCredential.md)
Uygulamayla ilişkili kimlik bilgilerinin listesini alır.

### [Get-AzADApplication](Get-AzADApplication.md)
Var olan Azure Active Directory uygulamalarını listeler.

### [Get-AzADGroup](Get-AzADGroup.md)
Active Directory gruplarına filtre uygular.

### [Get-AzADGroupMember](Get-AzADGroupMember.md)
Geçerli Kiracıdaki bir AD grubunun üyelerini listeler.

### [Get-AzADServicePrincipal](Get-AzADServicePrincipal.md)
Active Directory hizmet sorumlularına filtre uygular.

### [Get-AzADSpCredential](Get-AzADSpCredential.md)
Hizmet sorumlusu ile ilişkili kimlik bilgilerinin listesini alır.

### [Get-AzADUser](Get-AzADUser.md)
Active Directory kullanıcılarına filtre uygular.

### [Get-AzDenyAssignment](Get-AzDenyAssignment.md)
Belirtilen kapsamda Azure RBAC reddetme atamalarını listeler.
Varsayılan olarak, seçilen Azure aboneliğindeki tüm atamaları engeller.
Belirli bir kullanıcıya yönelik atamaları Reddet veya belirli bir kaynak grubundaki veya kaynağındaki atamaları engelle seçeneğini listelemek için ilgili parametreleri kullanın.

### [Get-AzDeployment](Get-AzDeployment.md)
Dağıtımı al

### [Get-AzDeploymentOperation](Get-AzDeploymentOperation.md)
Dağıtım işlemini alma

### [Get-Azdeploymentscrıpt](Get-AzDeploymentScript.md)
Dağıtım komut dosyalarını alır veya listeler.

### [Get-AzDeploymentScriptLog](Get-AzDeploymentScriptLog.md)
Dağıtım betiği yürütme günlüğünü alır.

### [Get-Azdeploymentwhatıya sonucu](Get-AzDeploymentWhatIfResult.md)
Abonelik kapsamındaki bir dağıtımın sonucu What-If ARM şablonunu alır. 

### [Get-AzLocation](Get-AzLocation.md)
Her konum için tüm konumları ve desteklenen kaynak sağlayıcılarını alır.

### [Get-AzManagedApplication](Get-AzManagedApplication.md)
Yönetilen uygulamaları alır

### [Get-AzManagedApplicationDefinition](Get-AzManagedApplicationDefinition.md)
Yönetilen uygulama tanımlarını alır

### [Get-AzManagementGroup](Get-AzManagementGroup.md)
Yönetim gruplarını alır

### [Get-AzManagementGroupDeployment](Get-AzManagementGroupDeployment.md)
Yönetim grubunda dağıtım alma

### [Get-AzManagementGroupDeploymentOperation](Get-AzManagementGroupDeploymentOperation.md)
Yönetim grubu dağıtımı için dağıtım işlemi alma

### [Get-Azmanagementgroupdeploymentwhatı, result](Get-AzManagementGroupDeploymentWhatIfResult.md)
Yönetim grubu kapsamındaki bir dağıtımın sonucu What-If ARM şablonunu alır. 

### [Get-Azpolicyalıas](Get-AzPolicyAlias.md)
Tanımlı ve verilen parametre değerleriyle eşleşen Azure sağlayıcısı kaynak türlerini alır ve bu kaynakları alır Get-AzPolicyAlias. Parametre sağlanmadıysa, diğer ad içeren tüm sağlayıcı kaynak türleri çıktı olacaktır.
-ListAvailable anahtarı, diğer adlar olmadan eşleşen tüm kaynak türlerini listeleyerek bu davranışı değiştirir.

### [Get-AzPolicyAssignment](Get-AzPolicyAssignment.md)
İlke atamalarını alır.

### [Get-AzPolicyDefinition](Get-AzPolicyDefinition.md)
İlke tanımlarını alır.

### [Get-AzPolicySetDefinition](Get-AzPolicySetDefinition.md)
İlke kümesi tanımlarını alır.

### [Get-AzProviderFeature](Get-AzProviderFeature.md)
Azure sağlayıcı özellikleri hakkında bilgi alır.

### [Get-AzProviderOperation](Get-AzProviderOperation.md)
Azure RBAC kullanarak güvenliği sağlanabilir bir Azure Kaynak sağlayıcısı için işlemleri alır.

### [Get-AzResource](Get-AzResource.md)
Kaynakları alır.

### [Get-AzResourceGroup](Get-AzResourceGroup.md)
Kaynak gruplarını alır.

### [Get-AzResourceGroupDeployment](Get-AzResourceGroupDeployment.md)
Kaynak grubundaki dağıtımları alır.

### [Get-AzResourceGroupDeploymentOperation](Get-AzResourceGroupDeploymentOperation.md)
Kaynak grubu dağıtım işlemini alır

### [Get-Azkaynakgrupyol](Get-AzResourceGroupDeploymentWhatIfResult.md)
Kaynak grubu kapsamındaki bir dağıtımın sonucu What-If ARM şablonunu alır. 

### [Get-AzResourceLock](Get-AzResourceLock.md)
Kaynak kilidini alır.

### [Get-Azkaynaksağlayıcı](Get-AzResourceProvider.md)
Kaynak sağlayıcısını alır.

### [Get-Azrol ataması](Get-AzRoleAssignment.md)
Belirtilen kapsamda Azure RBAC rol atamalarını listeler.
Varsayılan olarak, seçilen Azure aboneliğindeki tüm rol atamalarını listeler.
Belirli bir kullanıcıya yönelik atamaları listelemek veya belirli bir kaynak grubundaki veya kaynağındaki atamaları listelemek için ilgili parametreleri kullanın.

### [Get-Azroltanımı](Get-AzRoleDefinition.md)
Ödev için kullanılabilen tüm Azure RBAC rollerini listeler.

### [Get-AzTag](Get-AzTag.md)
Önceden tanımlanmış Azure etiketlerini alır | Kaynağın veya aboneliğin tüm etiket kümesini alır.

### [Get-Aztenantdağıtımı](Get-AzTenantDeployment.md)
Kiracı kapsamında dağıtım alma

### [Get-AzTenantDeploymentOperation](Get-AzTenantDeploymentOperation.md)
Kiracı kapsamındaki dağıtım için dağıtım işlemini alma

### [Get-Aztenantdeploymentwhadiresult](Get-AzTenantDeploymentWhatIfResult.md)
Kiracı kapsamındaki bir dağıtımın sonucu What-If ARM şablonunu alır. 

### [Invoke-Azkaynakeylemi](Invoke-AzResourceAction.md)
Kaynakta bir eylem çağırır.

### [Taşı-Azkaynak](Move-AzResource.md)
Kaynağı farklı bir kaynak grubuna veya aboneliğe taşıma.

### [New-AzADAppCredential](New-AzADAppCredential.md)
Var olan bir uygulamaya kimlik bilgisi ekler.

### [New-AzADApplication](New-AzADApplication.md)
Yeni bir Azure Active Directory uygulaması oluşturur.

### [New-AzADGroup](New-AzADGroup.md)
Yeni bir Active Directory grubu oluşturur.

### [Yeni-AzADServicePrincipal](New-AzADServicePrincipal.md)
Yeni bir Azure Active Directory hizmeti sorumlusu oluşturur.

### [Yeni-AzADSpCredential](New-AzADSpCredential.md)
Var olan hizmet sorumlusuna kimlik bilgileri ekler.

### [Yeni-AzADUser](New-AzADUser.md)
Yeni bir Active Directory kullanıcısı oluşturur.

### [New-AzDeployment](New-AzDeployment.md)
Dağıtım oluşturma

### [Yeni-AzManagedApplication](New-AzManagedApplication.md)
Azure yönetilen uygulaması oluşturur.

### [Yeni-AzManagedApplicationDefinition](New-AzManagedApplicationDefinition.md)
Yönetilen uygulama tanımı oluşturur.

### [New-AzManagementGroup](New-AzManagementGroup.md)
Yönetim grubu oluşturur

### [Yeni-AzManagementGroupDeployment](New-AzManagementGroupDeployment.md)
Yönetim grubunda dağıtım oluşturma

### [Yeni-AzManagementGroupSubscription](New-AzManagementGroupSubscription.md)
Yönetim grubuna abonelik ekler.

### [New-AzPolicyAssignment](New-AzPolicyAssignment.md)
İlke ataması oluşturur.

### [Yeni-Azilketanımı](New-AzPolicyDefinition.md)
İlke tanımı oluşturur.

### [New-AzPolicySetDefinition](New-AzPolicySetDefinition.md)
İlke kümesi tanımı oluşturur.

### [Yeni-aza kaynağı](New-AzResource.md)
Kaynak oluşturur.

### [Yeni-AzResourceGroup](New-AzResourceGroup.md)
Bir Azure Kaynak grubu oluşturur.

### [New-AzResourceGroupDeployment](New-AzResourceGroupDeployment.md)
Kaynak grubuna Azure dağıtımı ekler.

### [New-AzResourceLock](New-AzResourceLock.md)
Kaynak kilidi oluşturur.

### [Yeni-Azrol ataması](New-AzRoleAssignment.md)
Belirtilen RBAC rolü, belirtilen kapsamda belirtilen sorumluya atar.

### [Yeni-Azrol tanımı](New-AzRoleDefinition.md)
Azure RBAC 'de özel bir rol oluşturur.
Giriş olarak bir JSON rol tanımı dosyası veya PSRoleDefinition nesnesi sağlayın.
İlk olarak, temel rol tanımı nesnesi oluşturmak için Get-AzRoleDefinition komutunu kullanın.
Ardından özelliklerini gerektiği gibi değiştirin.
Son olarak, rol tanımını kullanarak özel bir rol oluşturmak için bu komutu kullanın.

### [New-AzTag](New-AzTag.md)
Önceden tanımlanmış bir Azure etiketi oluşturur veya varolan bir etikete değerler ekler | Kaynağın veya aboneliğin tüm etiket kümesini oluşturur veya güncelleştirir.

### [Yeni-Aztenantdağıtımı](New-AzTenantDeployment.md)
Kiracı kapsamında dağıtım oluşturma

### [Register-AzProviderFeature](Register-AzProviderFeature.md)
Hesabınızda bir Azure sağlayıcı özelliği kaydeder.

### [Register-AzResourceProvider](Register-AzResourceProvider.md)
Kaynak sağlayıcısını kaydeder.

### [Remove-AzADAppCredential](Remove-AzADAppCredential.md)
Uygulamadaki kimlik bilgilerini kaldırır.

### [Remove-AzADApplication](Remove-AzADApplication.md)
Azure Active Directory uygulamasını siler.

### [Remove-AzADGroup](Remove-AzADGroup.md)
Active Directory grubunu siler.

### [Remove-AzADGroupMember](Remove-AzADGroupMember.md)
Bir AD grubundan kullanıcıyı kaldırır.

### [Remove-AzADServicePrincipal](Remove-AzADServicePrincipal.md)
Azure Active Directory hizmet sorumlusunu siler.

### [Remove-AzADSpCredential](Remove-AzADSpCredential.md)
Hizmet sorumlusunun kimlik bilgilerini kaldırır.

### [Remove-AzADUser](Remove-AzADUser.md)
Active Directory kullanıcısını siler.

### [Remove-AzDeployment](Remove-AzDeployment.md)
Bir dağıtımı ve ilişkili işlemleri kaldırır

### [Remove-AzDeploymentScript](Remove-AzDeploymentScript.md)
Dağıtım betiğini ve ilişkili kaynaklarını kaldırır.

### [Remove-AzManagedApplication](Remove-AzManagedApplication.md)
Yönetilen uygulamayı kaldırır

### [Remove-AzManagedApplicationDefinition](Remove-AzManagedApplicationDefinition.md)
Yönetilen uygulama tanımını kaldırır

### [Remove-AzManagementGroup](Remove-AzManagementGroup.md)
Yönetim grubunu kaldırır

### [Remove-AzManagementGroupDeployment](Remove-AzManagementGroupDeployment.md)
Yönetim grubundaki bir dağıtımı ve ilişkili işlemleri kaldırır

### [Remove-AzManagementGroupSubscription](Remove-AzManagementGroupSubscription.md)
Bir yönetim grubundan aboneliği kaldırır.

### [Remove-AzPolicyAssignment](Remove-AzPolicyAssignment.md)
İlke atamasını kaldırır.

### [Remove-AzPolicyDefinition](Remove-AzPolicyDefinition.md)
İlke tanımını kaldırır.

### [Remove-AzPolicySetDefinition](Remove-AzPolicySetDefinition.md)
İlke kümesi tanımını kaldırır.

### [Remove-AzResource](Remove-AzResource.md)
Kaynağı kaldırır.

### [Remove-AzResourceGroup](Remove-AzResourceGroup.md)
Kaynak grubunu kaldırır.

### [Remove-AzResourceGroupDeployment](Remove-AzResourceGroupDeployment.md)
Kaynak grubu dağıtımını ve ilişkili işlemleri kaldırır.

### [Remove-AzResourceLock](Remove-AzResourceLock.md)
Kaynak kilidini kaldırır.

### [Remove-Azroleödev](Remove-AzRoleAssignment.md)
Belirli bir kapsamda belirli bir role atanmış olan belirtilen sorumluya rol atamasını kaldırır.

### [Remove-AzRoleDefinition](Remove-AzRoleDefinition.md)
Azure RBAC 'de özel bir rol siler.
Silinecek rol, rolün ID özelliği kullanılarak belirtilir.
Özel rolde yapılan rol atamaları varsa, silme işlemi başarısız olur.

### [Remove-AzTag](Remove-AzTag.md)
Önceden tanımlanmış Azure etiketlerini veya değerlerini siler | Kaynağın veya aboneliğin tüm etiket kümesini siler.

### [Remove-AzTenantDeployment](Remove-AzTenantDeployment.md)
Kiracı kapsamındaki bir dağıtımı ve ilişkili işlemleri kaldırır

### [Save-AzDeploymentScriptLog](Save-AzDeploymentScriptLog.md)
Dağıtım betiği yürütme günlüğünü diske kaydeder.

### [Save-AzDeploymentTemplate](Save-AzDeploymentTemplate.md)
Dağıtım şablonunu dosyaya kaydeder.

### [Save-AzManagementGroupDeploymentTemplate](Save-AzManagementGroupDeploymentTemplate.md)
Dağıtım şablonunu dosyaya kaydeder.

### [Save-AzResourceGroupDeploymentTemplate](Save-AzResourceGroupDeploymentTemplate.md)
Bir dosyaya kaynak grubu dağıtım şablonunu kaydeder.

### [Save-AzTenantDeploymentTemplate](Save-AzTenantDeploymentTemplate.md)
Dağıtım şablonunu dosyaya kaydeder.

### [Set-AzManagedApplication](Set-AzManagedApplication.md)
Yönetilen uygulamayı güncelleştirir

### [Set-AzManagedApplicationDefinition](Set-AzManagedApplicationDefinition.md)
Yönetilen uygulama tanımını güncelleştirir

### [Set-AzPolicyAssignment](Set-AzPolicyAssignment.md)
Bir ilke atamasını değiştirir.

### [Set-Azilketanımı](Set-AzPolicyDefinition.md)
İlke tanımını değiştirir.

### [Set-AzPolicySetDefinition](Set-AzPolicySetDefinition.md)
İlke kümesi tanımını değiştirme

### [Set-Azkaynak](Set-AzResource.md)
Bir kaynağı değiştirir.

### [Set-AzResourceGroup](Set-AzResourceGroup.md)
Kaynak grubunda değişiklik yapar.

### [Set-AzResourceLock](Set-AzResourceLock.md)
Kaynak kilidini değiştirir.

### [Set-Azroleödev](Set-AzRoleAssignment.md)
Rol atamasını güncelleştirme.

### [Set-Azroltanımı](Set-AzRoleDefinition.md)
Azure RBAC 'de özel bir rolü değiştirir.
Değiştirilmiş rol tanımını JSON dosyası veya PSRoleDefinition olarak sağlayın.
İlk olarak, değiştirmek istediğiniz özel rolü almak için Get-AzRoleDefinition komutunu kullanın.
Ardından, değiştirmek istediğiniz özellikleri değiştirin.
Son olarak bu komutu kullanarak rol tanımını kaydedin.

### [Stop-AzDeployment](Stop-AzDeployment.md)
Çalışan bir dağıtımı iptal etme

### [Stop-AzManagementGroupDeployment](Stop-AzManagementGroupDeployment.md)
Yönetim grubunda çalışan bir dağıtımı iptal etme

### [Stop-AzResourceGroupDeployment](Stop-AzResourceGroupDeployment.md)
Kaynak grubu dağıtımını iptal eder.

### [Stop-Aztenantdağıtımı](Stop-AzTenantDeployment.md)
Kiracı kapsamında çalışan bir dağıtımı iptal etme

### [Test-Azdağıtım](Test-AzDeployment.md)
Bir dağıtımı doğrular.

### [Test-AzManagementGroupDeployment](Test-AzManagementGroupDeployment.md)
Yönetim grubunda bir dağıtımı doğrular.

### [Test-AzResourceGroupDeployment](Test-AzResourceGroupDeployment.md)
Kaynak grubu dağıtımını doğrular.

### [Test-Aztenantdağıtımı](Test-AzTenantDeployment.md)
Kiracı kapsamındaki bir dağıtımı doğrular.

### [Unregister-AzProviderFeature](Unregister-AzProviderFeature.md)
Hesabınızdaki bir Azure sağlayıcı özelliğinin kaydını siler.

### [Unregister-Azkaynaksağlayıcı](Unregister-AzResourceProvider.md)
Kaynak sağlayıcının kaydını siler.

### [Güncelleştirme-AzADApplication](Update-AzADApplication.md)
Var olan bir Azure Active Directory uygulamasını güncelleştirir.

### [Güncelleştirme-AzADServicePrincipal](Update-AzADServicePrincipal.md)
Var olan bir Azure Active Directory hizmet sorumlusunu güncelleştirir.

### [Update-AzADUser](Update-AzADUser.md)
Var olan Active Directory kullanıcısını güncelleştirir.

### [Güncelleştirme-AzManagementGroup](Update-AzManagementGroup.md)
Yönetim grubunu güncelleştirir

### [Update-AzTag](Update-AzTag.md)
Kaynak veya abonelikteki etiket kümesini seçmeli olarak güncelleştirir.
