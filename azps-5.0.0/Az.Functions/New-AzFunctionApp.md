...
### Örnek 1: Merkezi ABD 'de bir tüketim PowerShell işlev uygulaması oluşturun.
```powershell
...
Bu komut, Merkezi ABD 'de bir tüketim PowerShell işlev uygulaması oluşturur.

### Örnek 2: hizmet planında barındırılan bir PowerShell işlev uygulaması oluşturun.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Bu komut, hizmet planında barındırılan bir PowerShell işlevi uygulaması oluşturur.

### Örnek 3: özel bir ACR görüntüsü kullanarak işlev uygulaması oluşturun.
```powershell
PS C:\> New-AzFunctionApp -Name MyUniqueFunctionAppName `
                          -ResourceGroupName MyResourceGroupName `
                          -PlanName MyPlanName `
...
Bu komut özel bir ACR görüntüsü kullanarak işlev uygulaması oluşturur.
...
### -Disableapplicationınsights
Uygulama oluşturma işlevi sırasında Application Insights kaynağı oluşturmayı devre dışı bırakın.
Hiçbir günlük kullanılamayacak.
...
### -Dockerımagename
Yalnızca Linux.
Docker kayıt defterinden kapsayıcı görüntü adı; örneğin, Publisher/image-Name: Tag.
...
Kapsayıcı kayıt defteri Kullanıcı adı ve parolası.
Özel kayıt defterleri için gereklidir.
...
Bu parametre için kabul edilebilir değerler şunlardır:-SystemAssigned-Useratandı
...
### -OSType
İşlev uygulamasını barındıracak işletim sistemi.
