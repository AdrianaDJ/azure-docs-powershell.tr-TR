...
## SYNOPSIS
Destek bileti oluşturur.

## INDEKI

### Createsupportbilet Withcontactdetailparameterset (varsayılan)
```
...
### Createsupportparameterwith, Tobjectparameterset
```
New-AzSupportTicket -Name <String> -Title <String> -Description <String> -ProblemClassificationId <String>
...
### Createteknisyen Alsupportbilet with, Tobjectparameterset
```
New-AzSupportTicket -Name <String> -Title <String> -Description <String> -ProblemClassificationId <String>
 -Severity <Severity> -CustomerContactDetail <PSContactProfile> [-ProblemStartTime <DateTime>]
...
### Createquotasupportbilet with, Tobjectparameterset
```
New-AzSupportTicket -Name <String> -Title <String> -Description <String> -ProblemClassificationId <String>
 -Severity <Severity> -CustomerContactDetail <PSContactProfile> [-ProblemStartTime <DateTime>]
...
### Createteknisyen Alsupportbilet Withcontactdetailparameterset
```
New-AzSupportTicket -Name <String> -Title <String> -Description <String> -ProblemClassificationId <String>
 -Severity <Severity> -CustomerFirstName <String> -CustomerLastName <String>
 -PreferredContactMethod <ContactMethod> -CustomerPrimaryEmailAddress <String>
...
### Createquotasupportbilet Withcontactdetailparameterset
```
New-AzSupportTicket -Name <String> -Title <String> -Description <String> -ProblemClassificationId <String>
 -Severity <Severity> -CustomerFirstName <String> -CustomerLastName <String>
 -PreferredContactMethod <ContactMethod> -CustomerPrimaryEmailAddress <String>
...
## Tanım
Bu cmdlet, faturalandırma, abonelik yönetimi, kota veya teknik sorunlar için destek bileti oluşturmak amacıyla kullanılabilir. Get-AzSupportService ve Get-AzSupportProblemClassification cmdlet 'lerini kullanarak Azure hizmetini tanımlar ve destek isteğinde bulunmasını istediğiniz sırayla buna karşılık gelen sorun sınıflandırmaları kullanılır. Aşağıdaki parametreleri belirtmeniz gerekir: 
...
CustomerContactDetail nesnesini oluşturmak için New-AzSupportContactProfileObject yardımcı cmdlet 'ini kullanabilirsiniz.

Bulut çözümü sağlayıcıları, müşterinin kiracısına oturum açıp *Csphometenantid* parametresini kullanarak kendi ev Kiracı kimliğini belirleyerek müşterinin abonelikleri için bir destek bileti oluşturabilir.

__Teknik biletler için:__

Kaynak adını belirtmek için, teknisyen kaynak kimliğini, *teknisyen* kaynak kimliği parametresini kullanarak belirtin. Aşağıdaki örneğe bakın. 

__Kota biletleri için:__

**COMPUTE VM çekirdekleri** , **toplu Iş** , **SQL veritabanı** ve **SQL veri ambarına** kota artışı istemek için, *quotatıayrıntı* nesnesi altında ek ayrıntılar sağlayın. Quotabilet ayrıntı nesnesi aşağıda açıklandığı gibi 3 özellikten oluşur. Ayrıntılı belgeler için lütfen [buraya tıklayın.](https://aka.ms/supportrpquotarequestpayload)
...
Çeşitli kota türleri için yük oluşturma hakkında ayrıntılı belgeler için lütfen [buraya tıklayın](https://aka.ms/supportrpquotarequestpayload)

## ÖRNEKLERDEN

### Örnek 1: fatura veya abonelik yönetimi destek bileti oluşturun. Destek istemek istediğiniz fatura veya abonelik yönetimi sorun sınıflandırması için doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanma 
```powershell
...
### Örnek 2: Windows için sanal makine kaynağı için teknik destek bileti oluşturun. Destek istemek istediğiniz Windows sorun sınıflandırması için sanal makinenin doğru GUID 'Lerini almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanma 
```powershell
...
### Örnek 3: belirli bir VM ailesi için sanal makine çekirdeğinin kotasını artıracak kota desteği bileti oluşturun. Kota işlemi VM çekirdeğinin sorun sınıflandırması için doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 4: bir toplu hesap için düşük öncelikli çekirdek kotasını artıracak kota desteği bileti oluşturun. Kota toplu Işlemi sorun sınıflandırmasını doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 5: bir toplu hesap için belirli bir VM ailesi için VM çekirdek kotasını artıracak bir kota desteği bileti oluşturun. Kota toplu Işlemi sorun sınıflandırmasını doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 6: bir toplu hesap için havuz kotasını yükseltmek amacıyla kota desteği bileti oluşturma. Kota toplu Işlemi sorun sınıflandırmasını doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 7: bir toplu hesap için etkin Işler ve iş zamanlamaları kotasını artıran bir kota desteği bileti oluşturun. Kota toplu Işlemi sorun sınıflandırmasını doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 8: bir aboneliğin toplu Iş hesabı sayısını yükseltmek için bir kota destek bileti oluşturun. Kota toplu Işlemi sorun sınıflandırmasını doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 9: SQL veritabanında Vseçma kotasını yükseltmek için bir kota desteği bileti oluşturun. Kota SQL veritabanı sorun sınıflandırması 'nın doğru GUID 'Lerini almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 10: SQL veritabanı için bir kota desteği bileti oluşturun. Kota SQL veritabanı sorun sınıflandırması 'nın doğru GUID 'Lerini almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 11: SQL veri ambarına yönelik Vseçval kotasını yükseltmek için bir kota desteği bileti oluşturun. Kota SQL tarih ambarı sorun sınıflandırması için doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 12: SQL veri ambarı için sunucular kotasını yükseltmek amacıyla kota desteği bileti oluşturma. Kota SQL veri ambarı sorun sınıflandırması için doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
PS C:\> New-AzSupportTicket -Name "test1" -Title "Test" -Description "Test" -Severity "minimal" -ProblemClassificationId "/providers/Microsoft.Support/services/{quota_service_guid}/problemClassifications/{sql_datawarehouse_problemClassification_guid}" -QuotaTicketDetail @{QuotaChangeRequestVersion = "1.0" ; QuotaChangeRequestSubType = "Servers" ; QuotaChangeRequests = (@{Region = "westus"; Payload = "{`"NewLimit`":200}"})} -CustomerContactDetail @{FirstName = "first" ; LastName = "last" ; PreferredTimeZone = "pacific standard time" ; PreferredSupportLanguage = "en-us" ; Country = "USA" ; PreferredContactMethod = "Email" ; PrimaryEmailAddress = "user@contoso.com"}
...
### Örnek 13: makine öğrenme hizmeti için düşük öncelikli çekirdek kotasını yükseltmek amacıyla kota desteği bileti oluşturun. Kota makine Learning hizmeti sorun sınıflandırması için doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 14: makine öğrenme hizmeti için belirli bir VM ailesi için VM çekirdek kotasını artıracak kota desteği bileti oluşturun. Kota makine Learning hizmeti sorun sınıflandırması için doğru GUID 'Leri almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 15: Azure SQL yönetilen örneğinin kotasını yükseltmek için bir kota desteği bileti oluşturun. Kota SQL yönetilen örnek hizmeti sorun sınıflandırması 'nın doğru GUID 'Lerini almak için Get-AzSupportService ve Get-AzSupportProblemClassification kullanın.
```powershell
...
### Örnek 16: Müştercontactdetail nesnesi yerine bireysel müşteri irtibat parametrelerini belirterek bir destek bileti oluşturun. 
```powershell
...
### Örnek 17: Azure 'dan 24 x 7 yanıtını içeren bir destek bileti oluşturun. 
```powershell
...
### Örnek 18: bulut çözüm sağlayıcısı (CSP) kullanıyorsanız, müşterinizin adına bir destek bileti oluşturun. CSP ilk önce kiracılarında oturum açın ve aşağıdaki örnekte gösterildiği gibi müşterinin kiracısına oturum açın. Böylece, destek bileti oluştururken kendi ev Kiracı kimliğini belirtmek için-CSPHomeTenantId parametresini kullanmaları gerekir.  
```powershell
...
### -Adtionalemapostaadresi
Ek e-posta adresleri.
Burada listelenen e-posta adresleri destek bileti hakkında herhangi bir yazışmadan kopyalanır.
...
### -CSPHomeTenantId
Bu, bulut çözümü sağlayıcısı kullanıcısının, müşterilerine destek bileti oluşturmaya çalışan ev kiracı kimliğidir.
...
### -Müştercontactdetail
SupportTicket kaynağıyla ilişkilendirilmiş müşteri iletişim bilgileri.
...
### -CustomerCountry
Müşteri ülkesi.
Bu, geçerli bir ISO Alpha-3 ülke kodu (ISO 3166) olmalıdır.
...
### -Müştertelefonnumarası
Müşteri telefon numarası.
Tercih edilen kişi yöntemi telefon ise bu gereklidir.
...
### -Müşterpreferredsupportsupport
Özel.
Burada listelenen desteklenen dillerden birinin geçerli bir dil-Contry kodu olması gerekir https://azure.microsoft.com/en-us/support/faq/ .
...
### -Müşterpreferredtimezone
Müşterinin tercih ettiği saat dilimi.
Bu geçerli bir System.TimeZoneInfo.Id değeri olmalıdır.
...
### -Müştergunlubir Yemapostaadresi
Müşteri birincil e-posta adresi.
...
### -Ad
Bu cmdlet 'in oluşturduğu destek anahtarının adı.
...
### -ProblemClassificationId
Her Azure hizmeti, karşılaştığınız sorun türüne karşılık gelen sorun sınıflandırması olarak bilinen bir sorun kategorisi kümesi içerir.
Bu parametre ProblemClassification kaynağının ARM kaynak kimliğidir.
...
### -Quotafdetail
Kota desteği anahtarının ek ayrıntıları.
...
### -Require24X7Response
Bu destek biletinin Azure 'dan 7x24 bir yanıt gerektirip gerektirmediğini gösterir.
...
### -Önem derecesi
Destek biletinin önem derecesi.
Bu, durumun ne olduğunu gösterir, bu da yanıt süresini, Azure 'daki teknik destek planının hizmet düzeyi sözleşmesine göre belirler.
...
### -Teknisyen
Bu, destek anahtarının oluşturulduğu Azure hizmet kaynağının kaynak kimliğidir (örneğin: bir sanal makine kaynağı veya HDInsight kaynağı).
