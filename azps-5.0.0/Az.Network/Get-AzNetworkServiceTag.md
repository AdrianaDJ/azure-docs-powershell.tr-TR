...
## Tanım
**Get-AzNetworkServiceTag** cmdlet 'i, hizmet etiketi bilgi kaynaklarının listesini alır.

Belirttiğiniz Azure bölgesi bilgilerinin sürüm için bir başvuru olarak kullanılacağını (konuma göre filtre olarak değil) unutmayın. Örneğin, belirtseniz bile, `-Location eastus2` tüm bölgelerde önek ayrıntılarını içeren, ancak aboneliğinizin ait olduğu bulut ile sınırlı olan hizmet etiketlerinin listesini alacaksınız (örneğin, genel, ABD hükümeti, Çin veya Almanya).
...
Komut, hizmet etiketi bilgi kaynaklarının listesini alır ve bunu değişkende depolar `serviceTags` .

### Örnek 2: AzureSQL için tüm adres öneklerini alma
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
İlk komut, hizmet etiketi bilgi kaynaklarının listesini alır ve bunu değişkende depolar `serviceTags` .
İkinci komut, SQL için bilgi kaynağı seçme listesine filtre uygular.

### Örnek 3: Batı ABD 2 için depolama alanı hizmet etiketi bilgi kaynağını alma
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
İlk komut, hizmet etiketi bilgi kaynaklarının listesini alır ve bunu değişkende depolar `serviceTags` .
Aşağıdaki komutlar, Batı ABD 2 ' de depolama için hizmet etiketi bilgilerini seçmek üzere listeye filtre uygulamak için çeşitli bir yol gösterir.

### Örnek 4: tüm genel hizmet etiketi bilgi kaynaklarını alma
```powershell
PS C:\> $serviceTags = Get-AzNetworkServiceTag -Location eastus2
...
İlk komut, hizmet etiketi bilgi kaynaklarının listesini alır ve bunu değişkende depolar `serviceTags` .
İkinci komut, listeyi yalnızca ayarlamadan bölgeyi seçmek için seçer.
...
### -Konum
Sürüm için başvuru olarak kullanılacak konum (konumu temel alan bir filtre olarak değil, tüm bölgelerde önek ayrıntılarını içeren, ancak aboneliğinizin ait olduğu bulut ile sınırlı olan hizmet etiketlerinin listesini alırsınız).
...
### Microsoft. Azure. Commands. Network. model. PSNetworkServiceTag
