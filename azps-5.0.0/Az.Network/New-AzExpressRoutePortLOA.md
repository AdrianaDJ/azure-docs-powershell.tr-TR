...
## SYNOPSIS
Hızlı yol bağlantı noktası için yetkilendirme belgesi harfini indirin.
...
## Tanım
New-AzExpressRoutePortLOA cmdlet, bir hızlı yol bağlantı noktası için bir yetkilendirme belgesini PDF biçiminde indirir.
...
' MyPort ' hızlı rota bağlantı noktası için yetkilendirme belgesi harfini indirin ve ' loa.pdf ' dosyasında saklayın.
...
### -CustomerName
Bu hızlı yol bağlantı noktasının atandığı müşterinin adı.
...
### -Hedef
Yetkilendirme harfinin depolanacağı çıkış FilePath.
...
### -ExpressRoutePort
Hızlı rota bağlantı noktası kaynağı.
...
### -PortName
Hızlı rota bağlantı noktası adı.
...
### -ResourceGroupName
Hızlı yol bağlantı noktasının kaynak grubu adı.
