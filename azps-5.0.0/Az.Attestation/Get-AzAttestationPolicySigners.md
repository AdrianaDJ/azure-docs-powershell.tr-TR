...
## Tanım
Get-AzAttestationPolicySigners cmdlet 'i Azure kanıtlama 'ndaki güvenilen ilke İmzalayanları alır.
...
Kaynak grubunda *PSH-test-RG* *kanıtlama sağlayıcısı için* güvenilen ilke İmzalayanları alır.  Bu kanıtlama sağlayıcısı için güvenilen imzalayanlar olmadığını unutmayın.
...
Grup *PSH-test-RG* kaynak grubundaki kanıtlama sağlayıcısı *pshtest2* için güvenilen ilke İmzalayanları alır.  Bu kanıt sağlayıcısı için güvenilen bir imzalayan olduğunu unutmayın.
...
Konum *Merkezi* 'Nde kanıt varsayılan sağlayıcısı için güvenilen ilke İmzalayanları alır.  Kanıtlama varsayılan sağlayıcısı için güvenilen imzalayanlar olmadığını unutmayın.
...
### -DefaultProvider
Bu, varsayılan kanıtlama sağlayıcısı isteğinin olduğunu belirtir.
...
### -Konum
Varsayılan kanıtlama sağlayıcısının konumunu belirtir.
...
### -Ad
Kanıt sağlayıcısının adını belirtir.
...
### Microsoft. Azure. Commands. kanıt. model. PSPolicySigners
