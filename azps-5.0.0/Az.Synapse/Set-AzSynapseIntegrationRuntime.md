...
### Setbyıntegrationruntimename (varsayılan)
```
...
## Tanım
**Set-AzSynapseIntegrationRuntime** cmdlet 'i belirli parametrelerle tümleştirme çalışma zamanını güncelleştirir.
...
Cmdlet, paylaşılan tümleştirme çalışma zamanını kullanmak için çalışma alanını ekler. Parametre kullanılırken `-SharedIntegrationRuntimeResourceId` de eklenmelidir `-Type` . Çalışma alanına cmdlet 'i çalıştırmadan önce tümleştirme çalışma zamanını kullanma izni verilmesi gerektiğini unutmayın.

## PARAMETRELERINE

### -AuthKey
Self-hosted Integration Runtime 'ın kimlik doğrulama anahtarı.
...
### -DataFlowComputeType
Veri akışı işini yürütecek veri akışı kümesinin işlem türü.
...
### -DataFlowCoreCount
Veri akışı işini yürütecek veri akışı kümesinin çekirdek sayısı.
...
### -DataFlowTimeToLive
Veri akışı işi yürütecek veri akışı kümesinin (dakika olarak) yaşam süresi.
...
### -Dataproxyıntegrationruntimename
Proxy olarak kullanılan Self-Hosted Integration Runtime adı.
...
Self-Hosted ile Azure-SSIS tümleştirmesi çalışma zamanı arasında veri taşırken kullanılacak hazırlama veri deposuna başvuruda bulunan Azure Blob depolama bağlantılı hizmet adı.
...
Verileri Self-Hosted ile Azure-SSIS tümleştirmesi arasında taşırken kullanılacak hazırlama verileri deposundaki yol, belirtilmemişse varsayılan bir kapsayıcı kullanılır.
...
### -Edition
Standart veya kurumsal olabilecek SSIS tümleştirmesi çalışma zamanı modülü, belirtilmemişse varsayılan standart olur.
...
### -ExpressCustomSetup
Özel kurulum komut dosyası olmadan yapılandırmaları ve üçüncü taraf bileşenlerini ayarlamak için kullanılan SIS Integration Runtime için hızlı özel kurulum.
...
### -LicenseType
SıR için seçmek istediğiniz lisans türü.
İki tür vardır: Licenseeklenmiş veya temel fiyat.
Azure Karma Kullanım Avantajı (AHUB) fiyatlandırması yeterliği Kazanmıyorsanız, lütfen temel fiyat seçeneğini belirleyin.
Yoksa, lütfen License'i seçin.
...
### -Publicıp
Tümleştirme çalışma zamanının kullanacağı statik genel IP adresleri.
...
Özel kurulum betiğini içeren Azure Blob kapsayıcısının SAS URI 'SI.
...
### -Sharedıntegrationruntimeresourceıd
Paylaşılan Self-hosted Integration Runtime 'ın kaynak kimliği.
...
### -VNetId
Tümleştirme çalışma zamanının katılacağı VNet KIMLIĞI.
