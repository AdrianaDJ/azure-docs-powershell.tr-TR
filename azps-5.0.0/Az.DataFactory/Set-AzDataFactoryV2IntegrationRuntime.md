...
## Tanım
Set-AzDataFactoryV2IntegrationRuntime cmdlet 'i belirli parametrelerle tümleştirme çalışma zamanını güncelleştirir.
...
### Örnek 2: Self ile barındırılan tümleştirme çalışma zamanını paylaşın.
```
PS C:\> Set-AzDataFactoryV2IntegrationRuntime -ResourceGroupName 'rg-test-dfv2' -DataFactoryName 'test-df-eu2' -Name 'test-selfhost-ir' `
...
Cmdlet, paylaşılan tümleştirme çalışma zamanını kullanmak için ADF 'yi ekler. Parametre kullanılırken `-SharedIntegrationRuntimeResourceId` de eklenmelidir `-Type` . Veri fabrikasına cmdlet 'i çalıştırmadan önce tümleştirme çalışma zamanını kullanma izninizin olması gerektiğini unutmayın.

### Örnek 3: ADF 'de Self-Hosted IR 'yi Azure-SSIS IR olarak yapılandırma.
```
...
Azure-SSA cmdlet 'i güncelleştirmesi, veri ara
...
Self-Hosted ile Azure-SSIS tümleştirmesi çalışma zamanı arasında verileri taşırken kullanılacak hazırlama veri deposuna başvuruda bulunan Azure Blob depolama bağlantılı hizmet adı
...
### -DataProxyStagingPath
Verileri Self-Hosted ile Azure-SSIN arasında taşırken kullanılacak hazırlama verileri deposundaki yol, belirtilmemişse varsayılan bir kapsayıcı kullanılır
...
### -LicenseType
SıR için seçmek istediğiniz lisans türü. İki tür vardır: Licenseeklenmiş veya temel fiyat. Azure Karma Kullanım Avantajı (AHUB) fiyatlandırması yeterliği Kazanmıyorsanız, lütfen temel fiyat seçeneğini belirleyin. Yoksa, lütfen License'i seçin.
