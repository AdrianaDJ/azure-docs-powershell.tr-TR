...
## SYNOPSIS
Bir SignalR hizmetinin ağ ACL 'sini güncelleyin.
...
## Tanım
Varsayılan eylem ve genel ve özel bağlantı için ağ ACL 'Leri de içinde olmak üzere bir SignalR hizmetinin ağ ACL 'sini güncelleyin.

## ÖRNEKLERDEN

### Genel ağ için restapi, istemc
```powershell
...
### Özel uç nokta bağlantısı için istemci bağlantısına ve sunucu bağlantısına izin verme
```powershell
...
### Ortak ağ ve özel uç nokta bağlantısı için istemci bağlantısını reddetme
```powershell
...
### -Iş
Cmdlet 'i arka plan işinde çalıştırın.
...
### -DefaultAction
SignalR ağ ACL 'lerin varsayılan eylemi, izin ver veya Reddet. Ağ ACL 'Lerini reddetmenize veya ağ ACL 'Lerine izin verip vermemeyeceğine karar verir. Örneğin, varsayılan eylem izin verişimizde, yalnızca Deny ACL 'Leri önemlidir.
...
### -PrivateEndpointName
Güncelleştirilecek özel uç noktalarının adları
...
### -PublicNetwork
Ortak ağ ACL 'Lerini güncelleştirme
...
### -ResourceGroupName
Kaynak grubu adı.
Belirtilmemişse, varsayılan değer kullanılacaktır.
...
### Microsoft. Azure. Commands. SignalR. modeller. Pssignalrnetworkcacls
