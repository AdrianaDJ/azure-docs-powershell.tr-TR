...
PeerAsn 65000 kullanarak Seattle olanağındaki tek bir bağlantıyla yeni bir doğrudan eşleme oluşturma

### Yeni Exchange eşlemesi oluşturma
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
Yeni bir Exchange eşlemesi oluşturma

### Eski eşlemeyi ARM eş'ya dönüştürme
```powershell
#Gets the ASN
PS C:> $asn = Get-AzPeerAsn -PeerName Contoso
...
### -DirectConnection
New-AzExchangePeeringConnection ve bu komuta boru kullanarak yeni bir doğrudan bağlantı oluşturun.
...
### -ExchangeConnection
Bu komut için New-AzExchangePeeringConnection ve boru kullanarak yeni bir Exchange bağlantıları oluşturun.
...
### -InputObject
Bu nesneyi almak için Get-AzLegacyPeering kullanın.
...
### -MicrosoftNetwork
Birlikte çalışmak istediğiniz Microsoft ağını seçin.
...
### -Peerasnresourceıd
Eş ASN kaynak kimliği. kimliği almak için Get-AzPeerAsn kullanın.
...
### -PeeringLocation
Azure bölgesinden farklı fiziksel konum.
Get-AzPeeringLocation türünde \<kind\> şehir adını anahtar olarak kullan 'ı kullanın.
...
### -SKU
Açıkça başka bir seçenek belirlenmedikçe Basic_Direct_Free veya Premium_Direct_Free seçin.
...
### Etiketli
Microsoft eşleme hizmetiyle ilişkilendirilecek Etiketler.
