...
### Removecatalogıtemaclentryforgroup
```
...
## Tanım
**Remove-Azdatalakeanalizleri Tic, Alogitemaclentry** cmdlet 'ı, Data Lake Analytics 'teki bir kataloğun veya katalog öğesinin erişim denetim LISTESINDEN (ACL) bir GIRIŞINI (ACE) kaldırır.

## ÖRNEKLERDEN

### Örnek 1: kataloğun Kullanıcı ACL 'sini kaldırma
```powershell
...
Bu komut, contosoadla hesabının PATTI Fuller için Katalog ACL 'sini kaldırır.

### Örnek 2: veritabanının kullanıcı ACL 'sini kaldırma
```powershell
...
Bu komut, contosoadla hesabının PATTI Tamlaştırıcı veritabanı ACL 'sini kaldırır.
...
### -Group
Grubun kataloğunun ACL girişini kaldır.
...
### -ObjectID
Kaldırılacak kullanıcının kimliği.
...
### -Kullanıcı
Kullanıcı için kataloğun ACL girişini kaldırma.
