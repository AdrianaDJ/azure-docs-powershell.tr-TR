...
## Tanım
**Yeni-Azapsananagementoperation** cmdlet 'ı bir API işlemi oluşturur.

## ÖRNEKLERDEN

### Örnek 1: bir API yönetim işlemi oluşturma
```powershell
...
### Örnek 2: istek ve yanıt ayrıntılarını içeren bir API yönetim işlemi oluşturma
```powershell
PS C:\>$apimContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Bu örnek, istek ve yanıt ayrıntılarını içeren bir API yönetim işlemi oluşturur.
...
### -Apırevision
API düzeltme tanıtıcısı. Bu parametre isteğe bağlıdır. Belirtilmemişse, işlem geçerli etkin API düzeltmesine eklenir.
...
### -Ad
Yeni API yönetim işleminin görünen adını belirtir.
...
### -İstek
API yönetim işleminin ayrıntılarını belirtir.
...
### -Yanıtlar
Olası bir API yönetim işlemi yanıtları dizisini belirtir.
...
### -TemplateParameters
*URL şablonunda* tanımlanan bir parametre dizisini belirtir.
Bu parametreyi belirtmezseniz, *URLTEMPLATE* temel alınarak varsayılan bir değer oluşturulur.
