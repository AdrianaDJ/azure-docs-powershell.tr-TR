...
## Tanım
**Export-Azapsananagementapı** cmdlet 'i, BIR Azure API yönetimi API 'sini desteklenen biçimlerden birinde dosyaya aktarır.
...
### Örnek 2: OpenApi 3,0 belirtim biçimindeki bir API 'YI JSON belgesi olarak dışarı aktarma
```powershell
PS C:\>$ApiMgmtContext = New-AzApiManagementContext -ResourceGroupName "Api-Default-WestUS" -ServiceName "contoso"
...
Bu komut, açık API biçiminde bir API tanımını JSON belgesi olarak dışarı aktarır
...
psdx_paramvalues Wadl, WSDL, Swagger, Openapı ve Openapıjson
