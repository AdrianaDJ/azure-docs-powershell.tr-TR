...
### Örnek 1: var olan kümeyi adıyla güncelleştirme
```powershell
...
Yukarıdaki komut, "testnewkustocluster" kaynak grubunda bulunan "" kusto kümesinin SKU adını güncelleştirir.

### Örnek 2: var olan kümeyi adıyla güncelleştirme
```powershell
...
Yukarıdaki komut, "testrg" kaynak grubunda bulunan "testnewkustocluster" kümesini müşteri yönetimli anahtarla güncelleştirir.
...
Kümenin disklerinin şifrelenip şifrelenmediğini belirten bir Boole değeri.
...
Çift şifrelemenin etkinleştirilip etkinleştirilmediğini belirten Boole değeri.
...
### -Enabletemizle
Temizleme işlemlerinin etkinleştirilip etkinleştirilmediğini belirten Boole değeri.
...
Akış gerektiren EST 'nin etkinleştirilip etkinleştirilmediğini belirten Boole değeri.
...
### -Identityuseratanandentity
Kusto kümesiyle ilişkili kullanıcı kimliklerinin listesi.
Kullanıcı kimliği sözlüğü anahtar başvuruları şu formda ARM kaynak kimlikleri olacaktır: '/subscriptions/{subscriptionId}/resourceGroups/{resourceGroupName}/providers/Microsoft.ManagedIdentity/userAssignedIdentities/{identityName} '.
...
Anahtar Kasası anahtarının adı.
...
Anahtar Kasası anahtarının sürümü.
...
Dil uzantılarının listesi.
Oluşturmak için LANGUAGEEXTENSIONVALUE özellikleri için Notlar bölümüne bakın ve karma tablo oluşturun.
...
### -Optimizedautoscaleısenabled
En iyileştirilmiş otomatik ölçeklendirme özelliğinin etkinleştirilip etkinleştirilmediğini belirten Boole değeri.
...
İzin verilen en fazla örnek sayısı.
...
İzin verilen minimum örnek sayısı.
...
Kümenin dış kiracıları.
Oluşturmak için, TRUSTEDEXTERNALTENANT özelliklerinin Notlar bölümüne bakın ve karma tablo oluşturun.
...
### -Virtualnetworkconfigurationdatamanagementpublicıpıd
Veri yönetimi hizmeti genel IP adresi kaynak kimliği.
...
### -Virtualnetworkconfigurationenginepublicıpıd
Altyapı hizmetinin genel IP adresi kaynak kimliği.
...
### Microsoft. Azure. PowerShell. cmdlet. kusto. modeller. Api20200614. ıluster
...
LANGUAGEEXTENSIONVALUE <ılanguageextension [] >: dil uzantılarının listesi.
  - `[Name <LanguageExtensionName?>]`: Dil uzantısının adı.

TRUSTEDEXTERNALTENANT <ITrustedExternalTenant [] >: kümenin dış kiracıları.
  - `[Value <String>]`: Dış kiracıyı temsil eden GUID.
