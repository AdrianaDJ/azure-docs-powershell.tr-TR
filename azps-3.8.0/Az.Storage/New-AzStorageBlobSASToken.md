...
## Tanım
**Yeni-AzStorageBlobSASToken** cmdlet 'i, bir Azure Depolama Blobu Için paylaşılan erişim IMZASı (SAS) belirteci oluşturur.
...
### Örnek 3: OAuth kimlik doğrulaması tabanlı bir depolama bağlamı olan Kullanıcı kimliği SAS belirteci oluşturma
```
...
Bu örnek, OAuth kimlik doğrulamasını temel alan depolama bağlamı içeren bir kullanıcı kimliği blob SAS belirteci oluşturur
...
**Cloudblob** nesnesi edinmek için [Get-azstorageblob](./Get-AzStorageBlob.md) cmdlet 'ini kullanın.
...
Depolama bağlamı OAuth kimlik doğrulamasına dayalı olduğunda, Kullanıcı kimliği blob SAS belirteci oluşturur.
...
Depolama bağlamı OAuth kimlik doğrulamasına dayalı olduğunda, geçerlilik süresi geçerli saatten 7 gün içinde olmalıdır ve geçerli saatten önce olmamalıdır.
...
### -İzin
Bir depolama blob 'una yönelik izinleri belirtir. Bu dizenin `rwd` (okuma, yazma ve silme için) bir dize olduğunu unutmayın. 
