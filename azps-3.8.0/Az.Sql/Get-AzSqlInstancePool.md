...
### ListBySubOrResourceGroupParameterSet (varsayılan)
```
...
### Getınstancepoolbyınstancepoolresourceıdentifierparameterset
```
Get-AzSqlInstancePool [-ResourceId] <String> [-DefaultProfile <IAzureContextContainer>] [<CommonParameters>]
```

## Tanım
**Get-Azsqlınstancepool** cmdlet 'i, bir veya daha fazla Azure SQL örneği havuzuyla ilgili bilgileri döndürür.
Yalnızca bu örnek havuzunun bilgilerini görmek için örnek havuzun adını belirtin.

## ÖRNEKLERDEN

### Örnek 1: müşteri aboneliği genelinde tüm örnek havuzlarını alma
```powershell
...
Bu komut, müşteri aboneliğindeki tüm örnek havuzlarıyla ilgili bilgileri alır.

### Örnek 2: kaynak grubundaki tüm örnek havuzlarını alma
```powershell
...
Bu komut, kaynak grubu resourcegroup01 içindeki örnek havuzlarıyla ilgili bilgileri alır.

### Örnek 3: örnek havuz hakkında bilgi alma
```powershell
...
Bu komut, örnek havuz instancePool0 hakkında bilgi alır.

### Örnek 4: örnek havuz kaynak kimliğini kullanarak örnek havuzu hakkında bilgi alma
```powershell
...
Bu komut, kaynak tanımlayıcısıyla birlikte örnek havuzu hakkında bilgi alır.
