...
### Bycertificatenadeniz Vseçversioninputobject
```
...
### Vites Avenadeniz Vseçversionresourceıd
```
Get-AzKeyVaultCertificate [-ResourceId] <String> [-Name] <String> [-Version] <String>
 [-DefaultProfile <IAzureContextContainer>] [<CommonParameters>]
```

### Bycertificateallversionsresourceıd
```
...
## Tanım
**Get-Azanahtarvaultcertificate** cmdlet 'ı, Azure Anahtar Kasası 'ndaki anahtar kasasından belirtilen sertifikayı veya sertifika sürümlerini alır.
...
### Örnek 2: sertifika alma ve PFX olarak kaydetme
Bu komut, ContosoKV01 adındaki anahtar kasasına TestCert01 adındaki sertifikayı alır. Sertifikayı pfx dosyası olarak indirmek için, aşağıdaki komutu çalıştırabilirsiniz. Bu komutlar SecretId öğesine erişir ve içeriği bir PFX dosyası olarak kaydeder.
...
### Örnek 3: silinmiş ancak bu Anahtar Kasası için temizlenmiş tüm sertifikaları edinin.
```powershell
...
### Örnek 4: silinen, ancak bu Anahtar Kasası için temizlenmediğim sertifikayı alır.
```powershell
...
### Örnek 5: filtreleme kullanarak sertifikaları listeleme
```powershell
...
### -Inclubaðlý olarak
Beklemedeki sertifikaların çıktıya eklenip eklenmeyeceğini belirtir
...
### Microsoft.Azure.Commands.KeyVault.Models.PSDEletedkeyvaultcertificateıdentityıtem
...
[Geri al-Azanahtarvaultsecretcertificate](./Undo-AzKeyVaultSecretCertificate.md)
