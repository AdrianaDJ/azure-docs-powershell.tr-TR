---
Module Name: Az.Batch
Module Guid: a8f00f40-1c1a-49b5-9db3-24076b75c3cf
Download Help Link: https://docs.microsoft.com/en-us/powershell/module/az.batch
Help Version: 4.0.0.0
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Batch/Batch/help/Az.Batch.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/master/src/Batch/Batch/help/Az.Batch.md
ms.openlocfilehash: ef071eefd84f18437a0b09582fe36fcc3b56e263
ms.sourcegitcommit: 6a91b4c545350d316d3cf8c62f384478e3f3ba24
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 04/21/2020
ms.locfileid: "93937877"
---
# Az.Batch modülü
## Tanım
Azure modülündeki Azure toplu Iş cmdlet 'leri Azure PowerShell 'de Microsoft Azure toplu hizmetlerini yönetmenizi sağlar.

## Az.Batch cmdlet 'Leri
### [Disable-Azbatchotomatik ölçeklendirme](Disable-AzBatchAutoScale.md)
Havuz otomatik ölçeklendirmeyi devre dışı bırakır.

### [Disable-AzBatchComputeNodeScheduling](Disable-AzBatchComputeNodeScheduling.md)
Belirtilen işlem düğümündeki görev zamanlamasını devre dışı bırakır.

### [Disable-AzBatchJob](Disable-AzBatchJob.md)
Toplu işi devre dışı bırakır.

### [Disable-Azbatchjobzamanlama](Disable-AzBatchJobSchedule.md)
Toplu iş çizelgesini devre dışı bırakır.

### [Enable-Azbatchotomatik ölçeklendirme](Enable-AzBatchAutoScale.md)
Havuzda otomatik ölçeklendirmeyi olanaklı kılar.

### [Enable-AzBatchComputeNodeScheduling](Enable-AzBatchComputeNodeScheduling.md)
Belirtilen işlem düğümündeki görev zamanlamasını mümkün kılar.

### [Enable-AzBatchJob](Enable-AzBatchJob.md)
Toplu işi etkinleştirilir.

### [Enable-Azbatchjobzamanlama](Enable-AzBatchJobSchedule.md)
Toplu iş çizelgesini mümkün kılar.

### [Enable-AzBatchTask](Enable-AzBatchTask.md)
Görevi yeniden etkinleştirir.

### [Get-AzBatchAccount](Get-AzBatchAccount.md)
Geçerli abonelikte bir toplu Iş hesabı alır.

### [Get-AzBatchAccountKey](Get-AzBatchAccountKey.md)
Bir toplu Iş hesabındaki anahtarları alır.

### [Get-AzBatchApplication](Get-AzBatchApplication.md)
Belirtilen uygulama hakkında bilgi alır.

### [Get-AzBatchApplicationPackage](Get-AzBatchApplicationPackage.md)
Bir toplu Iş hesabındaki uygulama paketi hakkında bilgi alır.

### [Get-AzBatchCertificate](Get-AzBatchCertificate.md)
Sertifikaları bir toplu Iş hesabında alır.

### [Get-AzBatchComputeNode](Get-AzBatchComputeNode.md)
Havuzdan toplu işlem düğümlerini alır.

### [Get-AzBatchJob](Get-AzBatchJob.md)
Toplu iş hesabı veya iş zamanlaması için toplu işleri alır.

### [Get-AzBatchJobPreparationAndReleaseTaskStatus](Get-AzBatchJobPreparationAndReleaseTaskStatus.md)
Toplu iş hazırlığını alır ve görev durumunu bırakın.

### [Get-Azbatchjobplan](Get-AzBatchJobSchedule.md)
Toplu iş zamanlamalarını alır.

### [Get-Azbatchjobistatistiğini](Get-AzBatchJobStatistic.md)
Bir toplu hesabın Proje Özeti istatistiklerini alır.

### [Get-AzBatchLocationQuota](Get-AzBatchLocationQuota.md)
Aboneliğiniz için toplu Iş hizmeti kotalarını verilen konumda alır.

### [Get-AzBatchNodeFile](Get-AzBatchNodeFile.md)
Toplu Iş düğümü dosyalarının özelliklerini alır.

### [Get-AzBatchNodeFileContent](Get-AzBatchNodeFileContent.md)
Bir toplu Iş düğümü dosyası alır.

### [Get-AzBatchPool](Get-AzBatchPool.md)
Belirtilen toplu hesap altında toplu Iş havuzlarını alır.

### [Get-AzBatchPoolNodeCount](Get-AzBatchPoolNodeCount.md)
Havuz kimliğine göre gruplandırılmış olarak düğüm başına toplu Iş düğümü sayısını alır.

### [Get-Azbatchpoolistatistiğini](Get-AzBatchPoolStatistic.md)
Toplu hesap için havuz Özeti istatistiklerini alır.

### [Get-AzBatchPoolUsageMetric](Get-AzBatchPoolUsageMetric.md)
Toplu hesap için havuz kullanım ölçümlerini alır.

### [Get-AzBatchRemoteDesktopProtocolFile](Get-AzBatchRemoteDesktopProtocolFile.md)
İşlem düğümünden RDP dosyası alır.

### [Get-AzBatchRemoteLoginSetting](Get-AzBatchRemoteLoginSetting.md)
İşlem düğümü için uzak oturum açma ayarlarını alır.

### [Get-Azbatchalt görevi](Get-AzBatchSubtask.md)
Belirtilen görevin alt görev bilgilerini alır.

### [Get-AzBatchTask](Get-AzBatchTask.md)
İş için toplu Iş görevlerini alır.

### [Get-AzBatchTaskCount](Get-AzBatchTaskCount.md)
Belirtilen iş için görev sayılarını alır.

### [Yeni-AzBatchAccount](New-AzBatchAccount.md)
Toplu hesap oluşturur.

### [Yeni-AzBatchAccountKey](New-AzBatchAccountKey.md)
Toplu hesap anahtarını yeniden oluşturur.

### [New-AzBatchApplication](New-AzBatchApplication.md)
Belirtilen toplu hesaba uygulama ekler.

### [New-AzBatchApplicationPackage](New-AzBatchApplicationPackage.md)
Bir toplu Iş hesabında bir uygulama paketi oluşturur.

### [Yeni-AzBatchCertificate](New-AzBatchCertificate.md)
Belirtilen toplu hesaba sertifika ekler.

### [Yeni-AzBatchComputeNodeUser](New-AzBatchComputeNodeUser.md)
Toplu işlem düğümündeki bir kullanıcı hesabı oluşturur.

### [Yeni-AzBatchJob](New-AzBatchJob.md)
Toplu Iş hizmetinde bir iş oluşturur.

### [Yeni-Azbatchjobplanlama](New-AzBatchJobSchedule.md)
Toplu Iş hizmetinde iş çizelgesi oluşturur.

### [Yeni-AzBatchPool](New-AzBatchPool.md)
Toplu Iş hizmetinde havuz oluşturur.

### [Yeni-AzBatchResourceFile](New-AzBatchResourceFile.md)
Tarafından kullanım için bir kaynak dosyası oluşturur `New-AzBatchTask` .

### [Yeni-AzBatchTask](New-AzBatchTask.md)
Bir iş altında toplu görev oluşturur.

### [Remove-AzBatchAccount](Remove-AzBatchAccount.md)
Toplu hesabı kaldırır.

### [Remove-AzBatchApplication](Remove-AzBatchApplication.md)
Bir uygulamayı toplu Iş hesabından siler.

### [Remove-AzBatchApplicationPackage](Remove-AzBatchApplicationPackage.md)
Uygulama paketi kaydını ve ikili dosyayı siler.

### [Remove-AzBatchCertificate](Remove-AzBatchCertificate.md)
Hesaptan bir sertifikayı siler.

### [Remove-AzBatchComputeNode](Remove-AzBatchComputeNode.md)
Bir havuzdan işlem düğümlerini kaldırır.

### [Remove-AzBatchComputeNodeUser](Remove-AzBatchComputeNodeUser.md)
Bir toplu işlem düğümünden Kullanıcı hesabını siler.

### [Remove-AzBatchJob](Remove-AzBatchJob.md)
Toplu iş siler.

### [Remove-Azbatchjobplan](Remove-AzBatchJobSchedule.md)
Toplu iş çizelgesini kaldırır.

### [Remove-AzBatchNodeFile](Remove-AzBatchNodeFile.md)
Görevin veya compute düğümünün düğüm dosyasını siler.

### [Remove-AzBatchPool](Remove-AzBatchPool.md)
Belirtilen toplu havuzu siler.

### [Remove-AzBatchTask](Remove-AzBatchTask.md)
Toplu Iş görevini siler.

### [Reset-AzBatchComputeNode](Reset-AzBatchComputeNode.md)
Belirtilen işlem düğümüne işletim sistemini yeniden yükler.

### [Restart-AzBatchComputeNode](Restart-AzBatchComputeNode.md)
Belirtilen işlem düğümünü yeniden başlatır.

### [Set-AzBatchAccount](Set-AzBatchAccount.md)
Bir toplu Iş hesabını güncelleştirir.

### [Set-AzBatchApplication](Set-AzBatchApplication.md)
Belirtilen uygulama için güncelleştirme ayarları.

### [Set-AzBatchComputeNodeUser](Set-AzBatchComputeNodeUser.md)
Toplu işlem düğümündeki bir hesabın özelliklerini değiştirir.

### [Set-AzBatchJob](Set-AzBatchJob.md)
Toplu işlemi güncelleştirir.

### [Set-Azbatchjobplan](Set-AzBatchJobSchedule.md)
İş zamanlaması ayarlar.

### [Set-AzBatchPool](Set-AzBatchPool.md)
Havuzun özelliklerini güncelleştirir.

### [Set-AzBatchTask](Set-AzBatchTask.md)
Görevin özelliklerini güncelleştirir.

### [Start-AzBatchComputeNodeServiceLogUpload](Start-AzBatchComputeNodeServiceLogUpload.md)
Compute node hizmeti günlük dosyalarını bir Azure depolama kapsayıcısına yükleyin.

### [Start-AzBatchPoolResize](Start-AzBatchPoolResize.md)
Havuzu yeniden boyutlandırmaya başlar.

### [Stop-Azbatchcertificatesilinmeye silme](Stop-AzBatchCertificateDeletion.md)
Sertifikanın başarısız silme işlemini iptal eder.

### [Stop-AzBatchJob](Stop-AzBatchJob.md)
Toplu işi durdurur.

### [Stop-Azbatchjobzamanlama](Stop-AzBatchJobSchedule.md)
Toplu iş çizelgesini durdurur.

### [Stop-AzBatchPoolResize](Stop-AzBatchPoolResize.md)
Havuz yeniden boyutlandırma işlemini durdurur.

### [Stop-AzBatchTask](Stop-AzBatchTask.md)
Toplu Iş görevini durdurur.

### [Test-Azbatchotomatik ölçeklendirme](Test-AzBatchAutoScale.md)
Havuzda otomatik ölçeklendirme formülünün sonucunu alır.

