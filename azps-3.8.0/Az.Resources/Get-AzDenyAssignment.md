...
### Denyassignmentivseçparameterset
```
...
## Tanım
Kapsamda etkin olan tüm atamaları engellemek için Get-AzDenyAssignment komutunu kullanın.
Parametre olmadan bu komut, aboneliğin altında yapılan tüm kabul atamalarını döndürür.
Bu liste, anapara için filtreleme parametreleri kullanılarak filtrelenebilir, ödev adını ve kapsamını reddedebilir.
...
Erişimin reddedildiği kapsam belirtilebilir.
Varsayılan olarak seçili aboneliği alır.
Reddet atamasının kapsamı, aşağıdaki parametre birleşimlerinin biri kullanılarak belirtilebilir.
Kapsam-bu,/Subscriptions/SubscriptionID ile başlayan tam nitelikli bir kapsamdır \< \> .
Bu, söz konusu kapsamda geçerli olan atamaları Reddet (.
ib.
ResourceGroupName-aboneliğin altındaki herhangi bir kaynak grubunun adı.
Bu, ödevleri belirtilen kaynak grubunda geçerli olacak şekilde filtreedecektir (.
d.
ResourceName, ResourceType, ResourceGroupName ve (isteğe bağlı) ParentResource-aboneliğin altındaki belirli bir kaynağı tanımlar ve bu kaynak kapsamda Reddet atamalarını etkin olarak süzer.
Abonelikteki belirli bir kullanıcı için erişimin reddedildiğini belirlemek için, ExpandPrincipalGroups anahtarını kullanın.
Bu, kullanıcıya atanan tüm atamaları ve kullanıcının üyesi olduğu grupları listeler.

## ÖRNEKLERDEN

### Örnek 1

Aboneliğin tüm atamalarını listeleme
...
john.doe@contoso.comTestRG ve üst kapsamındaki kullanıcıya yapılan tüm atamaları reddetme.
...
Belirtilen hizmet sorumlusunun tüm atamalarını reddetme
...
' Site1 ' Web sitesi kapsamındaki atamaları Reddet 'i alır.
...
Belirtilmişse, doğrudan kullanıcıya atanmış olan ve kullanıcının üye olduğu gruplara (geçişli) izin verme atamalarını getirir.
...
Filtreler belirtilen sorumluya yapılan tüm atamaları reddeder.
...
### -ResourceGroupName
Kaynak grubu adı.
Belirtilen kaynak grubunda geçerli olan atamaları Reddet listesini listeler.
ResourceName, ResourceType ve ParentResource parametreleriyle birlikte kullanıldığında, komut listeleri, kaynak grubundaki kaynaklarda atamaları reddeder.
...
Komut filtreleri bu kapsamda geçerli olan tüm atamaları reddeder.
...
Filtreler belirtilen Azure AD uygulamasına yapılan tüm atamaları reddeder.
...
Filtreler belirtilen kullanıcıya yapılan tüm atamaları reddeder.
