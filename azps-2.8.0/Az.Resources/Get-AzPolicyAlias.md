...
## Tanım
**Get-Azpolicyalıas** cmdlet 'i, ilke diğer adlarının bir listesini alır.
İlke diğer adları, kaynak türü özelliklerine başvuruda bulunmak için Azure Ilkesi tarafından kullanılır.
Kaynak türünün veya diğer adlarının çeşitli özelliklerini eşleştirerek listede öğeleri sınırlayan parametreler sağlanır.
Verilen eşleşme değeri, hedef dize büyük/küçük harf duyarsız karşılaştırmayı kullanarak içeriyorsa eşleşir.
...
Diğer adı olan tüm sağlayıcı kaynak türlerini listeler.
...
Diğer ad içermeyen olanlar da içinde olmak üzere tüm sağlayıcı kaynak türlerini listeler.
...
Ad alanı ' COMPUTE ' ile eşleşen ve diğer ad içeren tüm sağlayıcı kaynak türlerini listeler.
...
Kaynak türü ' sanal ' ile eşleşen ve diğer ad içeren tüm sağlayıcı kaynak türlerini listeler.
...
Kaynak türü, diğer adları olmayan ' sanal ' ile eşleşen tüm sağlayıcı kaynak türlerini listeler.
...
Ad alanı ' COMPUTE ' ve kaynak türüyle eşleşen tüm sağlayıcı kaynak türlerini listeler ' sanal ' ile eşleşiyor ve bir diğer ad içeriyor.
Not:-NamespaceMatch ve-Resourcettypeınfo, diğerleri dahil olmak üzere özel eşleşmeler sağlar.
...
' Sanal ' ile eşleşen diğer ad içeren tüm sağlayıcı kaynak türlerini listeler.
...
Eşleşen bir diğer ad veya ' ağ ' ile eşleşen bir diğer ad içeren tüm sağlayıcı kaynak türlerini listeler.
...
Alpha API sürümü ile tüm sağlayıcı kaynak türlerini listeler veya Alfa API sürümü olan diğer ad içerir.

## PARAMETRELERINE

### -Diğerad eşleştirme
Adı bu değerle eşleşen diğer adlara sahip çıkış öğelerine dahildir.
...
### -Apıversion
Ayarlandığında, kullanılacak kaynak sağlayıcısı API sürümünü gösterir. Belirtilmemişse API sürümü otomatik olarak en son kullanılabilir.
...
### -Apıversionmatch
Kaynak türlerinin veya diğer adlarının eşleşen API sürümüne sahip olduğu çıkış öğelerine dahildir.
...
### -ListAvailable
Diğer ad içeren ve bulunmayan çıkış eşleştirmesi öğeleri içerir.
...
### -LocationMatch
Kaynak türlerinin eşleşen bir konumuna sahip olduğu çıkış öğelerine dahildir.
...
### -NamespaceMatch
Çıktıyı, ad alanı bu değerle eşleşen öğelerle sınırlandırır.
...
### -PathMatch
Bu değerle eşleşen bir yol içeren diğer ad içeren çıkış öğelerine ekler.
...
### -Resourcettypeınfo
Çıktıyı, kaynak türü bu değerle eşleşen öğelerle sınırlar.
...
* Diğer adları veya diğer herhangi bir özelliği genişletmek için, çıktıyı boru yapın `select -ExpandProperty <property>` . Örneğin: `Get-AzPolicyAlias -NamespaceMatch 'Microsoft.Cache' -ApiVersionMatch 'alpha' | select -ExpandProperty Aliases | select -Property Name -ExpandProperty Paths`

* Çıktıda ek özellikler kullanılabilir ve çıktının boru tarafından görüntülenebilir `Format-List` . Örneğin: `Get-AzPolicyAlias -NamespaceMatch 'Web' -ResourceTypeMatch site -PathMatch cert | Format-List`
