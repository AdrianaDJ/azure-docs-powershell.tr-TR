...
## Tanım
Remove-AzPrivateDnsRecordConfig cmdlet 'i bir kayıt kümesinden özel bir etki alanı adı sistemi (DNS) kaydını kaldırır. Kayıt kümesi nesnesi bir çevrimdışı nesnedir ve değişiklikleri Microsoft Azure özel DNS hizmetine kalıcı hale getirmek için Set-AzPrivateDnsRecordSet cmdlet 'i çalıştırana kadar, özel DNS yanıtlarını değiştirmiyor. Kaydı kaldırmak için, bu kayıt türü için tüm alanların tam olarak eşleşmesi gerekir. SOA kayıtlarını ekleyemezsiniz veya kaldıramazsınız. SOA kayıtları, özel bir DNS bölgesi oluşturulduğunda ve özel DNS bölgesi silindiğinde otomatik olarak silindiğinde otomatik olarak oluşturulur. RecordSet nesnesini bu cmdlet 'e parametre olarak veya Pipeline işlecini kullanarak geçirebilirsiniz.
...
Bu örnekte, var olan kayıt kümesinden bir kayıt kaldırılır. Bu, kayıt kümesindeki tek kayıttır, sonuç boş bir kayıt kümesidir. Kayıt kümesini tümüyle kaldırmak için bkz: Remove-AzPrivateDnsRecordSet.
...
Bu örnekte, varolan bir kayıt kümesinden AAAA kaydı kaldırılır. Bu, kayıt kümesindeki tek kayıttır, sonuç boş bir kayıt kümesidir. Kayıt kümesini tümüyle kaldırmak için bkz: Remove-AzPrivateDnsRecordSet.
...
Bu örnekte, var olan kayıt kümesinden bir CNAME kaydı kaldırılır. CNAME kayıt kümesi en çok bir kayıt içerebileceğinden, sonuç boş bir kayıt kümesidir.
...
Bu örnekte, var olan kayıt kümesinden bir MX kaydı kaldırılır. "@" Kayıt adı, bölge Apex bir kayıt kümesini gösterir. Bu, kayıt kümesindeki tek kayıttır, sonuç boş bir kayıt kümesidir. Kayıt kümesini tümüyle kaldırmak için bkz: Remove-AzPrivateDnsRecordSet.

### Örnek 5: kayıt kümesinden PTR kaydını kaldırma
```powershell
...
Bu örnekte, var olan kayıt kümesinden bir PTR kaydı kaldırılır. Bu, kayıt kümesindeki tek kayıttır, sonuç boş bir kayıt kümesidir. Kayıt kümesini tümüyle kaldırmak için bkz: Remove-AzPrivateDnsRecordSet.

### Örnek 6: kayıt kümesinden SRV kaydını kaldırma
```powershell
...
Bu örnekte, var olan kayıt kümesinden bir SRV kaydı kaldırılır. Bu, kayıt kümesindeki tek kayıttır, sonuç boş bir kayıt kümesidir. Kayıt kümesini tümüyle kaldırmak için bkz: Remove-AzPrivateDnsRecordSet.

### Örnek 7: kayıt kümesinden TXT kaydını kaldırma
```powershell
...
Bu örnekte, var olan kayıt kümesinden bir TXT kaydı kaldırılır. Bu, kayıt kümesindeki tek kayıttır, sonuç boş bir kayıt kümesidir. Kayıt kümesini tümüyle kaldırmak için bkz: Remove-AzPrivateDnsRecordSet.

## PARAMETRELERINE

### -CNAME
Kaldırılacak CNAME kaydının kurallı adı.
...
### -Exchange
Kaldırılacak MX kaydının posta Exchange ana bilgisayarı.
Bölgenin adına göreli olmamalıdır.
Sonlandırıcı nokta olmamalıdır
...
### -Ipv4Address
Kaldırılacak kaydın IPv4 adresi.
...
### -IPv6Adresi
Kaldırılacak AAAA kaydının IPv6 adresi.
...
### -Bağlantı noktası
Kaldırılacak SRV kaydının bağlantı noktası numarası.
...
### -Tercih
Kaldırılacak MX kaydının tercih değeri.
...
### -Öncelik
Kaldırılacak SRV kaydının öncelik değeri.
...
### -Ptrdname
Kaldırılacak PTR kaydının hedef ana bilgisayarı.
Bölgenin adına göreli olmamalıdır.
Sonlandırıcı nokta olmamalıdır
...
### -Kayıt kümesi
Kaydın kaldırılacağı kayıt kümesi.
...
### -Hedef
Kaldırılacak SRV kaydının hedef ana bilgisayarı.
Bölgenin adına göreli olmamalıdır.
Sonlandırıcı nokta olmamalıdır
...
### -Değer
Kaldırılacak TXT kaydının metin değeri.
...
### -Kalınlık
Kaldırılacak SRV kaydının ağırlık değeri.
