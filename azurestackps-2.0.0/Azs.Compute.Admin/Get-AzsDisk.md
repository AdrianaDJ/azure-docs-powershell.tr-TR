...
### Örnek 1: tüm diskleri Al 
```powershell
PS C:\> Get-AzsDisk
```

Parametresiz, `Get-AzsDisk` tüm diskleri listeler.

### Örnek 2: ada göre disk alma
```powershell
...
Bir diski `Name` alacak şekilde belirtin.

### Örnek 3: belirtilen sayıda disk edinme
```powershell
...
`Count`Belirli sayıda diski almak için parametreyi kullanın.

### Örnek 4: belirli konumdaki tüm diskleri alma
```powershell
...
`ScaleUnit` `VolumeLabel` Belirli konumdaki tüm diskleri listelemek için veya parametresini kullanma
...
### -ScaleUnit
Kaynağın ait olduğu ölçeklendirme birimi.
...
### -Usersubscriptionıd
Kaynağın ait olduğu kullanıcı aboneliği kimliği.
...
### -Birimetiketi
Kaynağın ait olduğu birimin Birim etiketi.
...
### Microsoft. Azure. PowerShell. cmdlet. ComputeAdmin. modeller. Api20180730Preview. ıdisk
