---
Module Name: AzureRM.SiteRecovery
Module Guid: d1de7560-48e1-48f3-bc8c-4eea3af2bbe1
Download Help Link:
  object Object: 
Help Version:
  object Object: 
Locale: en-US
content_git_url: https://github.com/Azure/azure-powershell/blob/preview/src/ResourceManager/SiteRecovery/Commands.SiteRecovery/help/AzureRM.SiteRecovery.md
original_content_git_url: https://github.com/Azure/azure-powershell/blob/preview/src/ResourceManager/SiteRecovery/Commands.SiteRecovery/help/AzureRM.SiteRecovery.md
ms.openlocfilehash: 15d37c7f310fb3cce8188d1c71a06569d82c1b86
ms.sourcegitcommit: 43f4bdf2a59dd82fd881512aa9761bf72eb5703c
ms.translationtype: MT
ms.contentlocale: tr-TR
ms.lasthandoff: 04/23/2019
ms.locfileid: "93570970"
---
# AzureRM. SiteRecovery modülü
## Tanım
Bu konuda, Azure Site kurtarma cmdlet 'Lerinin yardım konularını görüntüler.

## AzureRM. Siterecoçok cmdlet 'Leri
### [Düzenle-AzureRmSiteRecoveryRecoveryPlan](Edit-AzureRmSiteRecoveryRecoveryPlan.md)
Site kurtarma planını düzenler.

### [Get-AzureRmSiteRecoveryFabric](Get-AzureRmSiteRecoveryFabric.md)
Bir Azure Site kurtarma yapısı özelliklerini edinin.

### [Get-AzureRmSiteRecoveryJob](Get-AzureRmSiteRecoveryJob.md)
Geçerli site kurtarma Kasası için işlem bilgilerini alır.

### [Get-AzureRmSiteRecoveryNetwork](Get-AzureRmSiteRecoveryNetwork.md)
Geçerli kasa için site kurtarma tarafından yönetilen ağlar hakkında bilgi alır.

### [Get-AzureRmSiteRecoveryNetworkMapping](Get-AzureRmSiteRecoveryNetworkMapping.md)
Geçerli kasa için site kurtarma ağ eşlemeleri hakkında bilgi alır.

### [Get-AzureRmSiteRecoveryPolicy](Get-AzureRmSiteRecoveryPolicy.md)
Site Recovery koruma ilkelerini alır.

### [Get-AzureRmSiteRecoveryProtectableItem](Get-AzureRmSiteRecoveryProtectableItem.md)
Koruma kapsayıcısındaki korunabilir öğeleri edinin.

### [Get-AzureRmSiteRecoveryProtectionContainer](Get-AzureRmSiteRecoveryProtectionContainer.md)
Geçerli site kurtarma Kasası için koruma kapsayıcılarını alır.

### [Get-AzureRmSiteRecoveryProtectionContainerMapping](Get-AzureRmSiteRecoveryProtectionContainerMapping.md)
Azure Site Recovery koruma kapsayıcısı eşleştirmelerini alır.

### [Get-AzureRmSiteRecoveryProtectionEntity](Get-AzureRmSiteRecoveryProtectionEntity.md)
Geçerli site kurtarma kasasındaki korunabilir veya korunan varlıkların listesini alır.

### [Get-AzureRmSiteRecoveryRecoveryPlan](Get-AzureRmSiteRecoveryRecoveryPlan.md)
Site kurtarma 'da kurtarma planı alır.

### [Get-AzureRmSiteRecoveryRecoveryPoint](Get-AzureRmSiteRecoveryRecoveryPoint.md)
Çoğaltma korumalı öğe için kullanılabilir kurtarma noktalarını alır.

### [Get-AzureRmSiteRecoveryReplicationProtectedItem](Get-AzureRmSiteRecoveryReplicationProtectedItem.md)
Azure Site Recovery korumalı öğelerinin özelliklerini alır.

### [Get-AzureRmSiteRecoveryServer](Get-AzureRmSiteRecoveryServer.md)
Geçerli kasaya kaydedilen site kurtarma sunucularıyla ilgili bilgileri alır.

### [Get-AzureRmSiteRecoveryServicesProvider](Get-AzureRmSiteRecoveryServicesProvider.md)
Kasadaki Azure Site Recovery sağlayıcılarından bilgi alır.

### [Get-AzureRmSiteRecoverySite](Get-AzureRmSiteRecoverySite.md)
Site kurtarma kasasından Hyper-V sitelerini alır.

### [Get-AzureRmSiteRecoveryStorageClassification](Get-AzureRmSiteRecoveryStorageClassification.md)
Site kurtarmada depolama sınıflandırmaları alır.

### [Get-AzureRmSiteRecoveryStorageClassificationMapping](Get-AzureRmSiteRecoveryStorageClassificationMapping.md)
Site kurtarmada bir depolama sınıflandırması eşlemesi alır.

### [Get-AzureRmSiteRecoveryVault](Get-AzureRmSiteRecoveryVault.md)
Geçerli aboneliğin site kurtarma şaşırmasını alır.

### [Get-AzureRmSiteRecoveryVaultSettings](Get-AzureRmSiteRecoveryVaultSettings.md)
Geçerli site kurtarma Kasası için ayarlar bilgilerini alır.

### [Get-AzureRmSiteRecoveryVaultSettingsFile](Get-AzureRmSiteRecoveryVaultSettingsFile.md)
Site Recovery kasa ayarları dosyasını alır.

### [Get-AzureRmSiteRecoveryVM](Get-AzureRmSiteRecoveryVM.md)
Site kurtarma ile yönetilen sanal makineler hakkında bilgi alır.

### [İçeri aktarma-AzureRmSiteRecoveryVaultSettingsFile](Import-AzureRmSiteRecoveryVaultSettingsFile.md)
Site Recovery kasa ayarları dosyasını içeri aktarır.

### [New-AzureRmSiteRecoveryFabric](New-AzureRmSiteRecoveryFabric.md)
Azure Site kurtarma yapısı oluşturur.

### [New-AzureRmSiteRecoveryNetworkMapping](New-AzureRmSiteRecoveryNetworkMapping.md)
Sanal ağlar arasında eşleme oluşturur.

### [New-AzureRmSiteRecoveryPolicy](New-AzureRmSiteRecoveryPolicy.md)
Site kurtarma çoğaltma ilkesi oluşturur.

### [New-AzureRmSiteRecoveryProtectionContainerMapping](New-AzureRmSiteRecoveryProtectionContainerMapping.md)
Bir ilkeyi bir koruma kapsayıcısına ilişkilendirerek Azure Site Recovery koruma kapsayıcısı eşlemesi oluşturur.

### [New-AzureRmSiteRecoveryRecoveryPlan](New-AzureRmSiteRecoveryRecoveryPlan.md)
Site kurtarma 'da site kurtarma planı oluşturur.

### [New-AzureRmSiteRecoveryReplicationProtectedItem](New-AzureRmSiteRecoveryReplicationProtectedItem.md)
Korumalı öğe oluşturarak korunabilir bir öğe için çoğaltmayı olanaklı kılar

### [New-AzureRmSiteRecoverySite](New-AzureRmSiteRecoverySite.md)
Site kurtarma sitesi oluşturur.

### [New-AzureRmSiteRecoveryStorageClassificationMapping](New-AzureRmSiteRecoveryStorageClassificationMapping.md)
Site kurtarmada bir depolama sınıflandırması eşlemesi oluşturur.

### [New-AzureRmSiteRecoveryVault](New-AzureRmSiteRecoveryVault.md)
Site Recovery Services Kasası oluşturur.

### [Remove-AzureRmSiteRecoveryFabric](Remove-AzureRmSiteRecoveryFabric.md)
Bir Azure Site kurtarma yapısı kaldırır.

### [Remove-AzureRmSiteRecoveryNetworkMapping](Remove-AzureRmSiteRecoveryNetworkMapping.md)
Geçerli site kurtarma kasasından Ağ eşlemesini kaldırır.

### [Remove-AzureRmSiteRecoveryPolicy](Remove-AzureRmSiteRecoveryPolicy.md)
Site kurtarma çoğaltma ilkesini kaldırır.

### [Remove-AzureRmSiteRecoveryProtectionContainerMapping](Remove-AzureRmSiteRecoveryProtectionContainerMapping.md)
Azure Site Recovery koruma kapsayıcısı eşlemesini kaldırır.

### [Remove-AzureRmSiteRecoveryRecoveryPlan](Remove-AzureRmSiteRecoveryRecoveryPlan.md)
Site kurtarma planını site kurtarmadan kaldırır.

### [Remove-AzureRmSiteRecoveryReplicationProtectedItem](Remove-AzureRmSiteRecoveryReplicationProtectedItem.md)
Azure Site Recovery çoğaltması korumalı öğeyi kaldırır.

### [Remove-AzureRmSiteRecoveryServer](Remove-AzureRmSiteRecoveryServer.md)
Geçerli kasadan site kurtarma sunucusunu kaldırır.

### [Remove-AzureRmSiteRecoveryServicesProvider](Remove-AzureRmSiteRecoveryServicesProvider.md)
Azure Site Recovery Hizmetleri sağlayıcısını kaldırır.

### [Remove-AzureRmSiteRecoverySite](Remove-AzureRmSiteRecoverySite.md)
Geçerli kasadan site kurtarma sitesini kaldırır.

### [Remove-AzureRmSiteRecoveryStorageClassificationMapping](Remove-AzureRmSiteRecoveryStorageClassificationMapping.md)
Bir depolama sınıflandırması eşlemesini site kurtarması 'ndan kaldırır.

### [Remove-AzureRmSiteRecoveryVault](Remove-AzureRmSiteRecoveryVault.md)
Site kurtarma kasasından çıkarır.

### [Restart-AzureRmSiteRecoveryJob](Restart-AzureRmSiteRecoveryJob.md)
Azure Site kurtarma işini yeniden başlatır.

### [Özgeçmiş-AzureRmSiteRecoveryJob](Resume-AzureRmSiteRecoveryJob.md)
Askıya alınan bir site kurtarma işini sürdürür.

### [Set-AzureRmSiteRecoveryProtectionEntity](Set-AzureRmSiteRecoveryProtectionEntity.md)
Site kurtarma koruma varlığının durumunu ayarlar.

### [Set-AzureRmSiteRecoveryReplicationProtectedItem](Set-AzureRmSiteRecoveryReplicationProtectedItem.md)
Çoğaltma korumalı öğenin hedef ağ ve sanal makine boyutu gibi kurtarma özelliklerini ayarlar.

### [Set-AzureRmSiteRecoveryVaultSettings](Set-AzureRmSiteRecoveryVaultSettings.md)
Diğer işlemler için site kurtarma Kasası bağlamını ayarlar.

### [Set-AzureRmSiteRecoveryVM](Set-AzureRmSiteRecoveryVM.md)
Site kurtarma koruma varlığının kurtarma tarafı seçeneklerini ayarlar.

### [Start-Azurermsitereto Plyrecoverypoint](Start-AzureRmSiteRecoveryApplyRecoveryPoint.md)
Yük devretme işlemini uygulamadan önce korunan bir öğenin kurtarma noktasını değiştirir.

### [Start-AzureRmSiteRecoveryCommitFailoverJob](Start-AzureRmSiteRecoveryCommitFailoverJob.md)
Site kurtarma nesnesi için yük devretmeyi Yürüt eylemini başlatır.

### [Start-Azurermsitere, Yplannedfailoverjob](Start-AzureRmSiteRecoveryPlannedFailoverJob.md)
Bir site kurtarma planlı yük devretme işlemini başlatır.

### [Start-AzureRmSiteRecoveryPolicyAssociationJob](Start-AzureRmSiteRecoveryPolicyAssociationJob.md)
Site kurtarma çoğaltması ilkesi ilişki işini başlatır.

### [Start-AzureRmSiteRecoveryPolicyDissociationJob](Start-AzureRmSiteRecoveryPolicyDissociationJob.md)
Site kurtarma koruma konteyneriyle ilişkilendirilmiş bir çoğaltma ilkesinde ilişkilendirme işini başlatır.

### [Start-AzureRmSiteRecoveryTestFailoverJob](Start-AzureRmSiteRecoveryTestFailoverJob.md)
Site kurtarma koruma varlığı için test yük devretmesi başlatır.

### [Start-AzureRmSiteRecoveryUnplannedFailoverJob](Start-AzureRmSiteRecoveryUnplannedFailoverJob.md)
Site Recovery koruması varlığı veya kurtarma planı için planlanmayan yük devretmeyi başlatır.

### [Stop-AzureRmSiteRecoveryJob](Stop-AzureRmSiteRecoveryJob.md)
Site kurtarma işini durdurur.

### [Güncelleştirme-AzureRmSiteRecoveryPolicy](Update-AzureRmSiteRecoveryPolicy.md)
{{Update-AzureRmSiteRecoveryPolicy açıklamasını el ile girin}}

### [Güncelleştirme-AzureRmSiteRecoveryProtectionDirection](Update-AzureRmSiteRecoveryProtectionDirection.md)
Site kurtarma nesnesinin korunması için kaynak ve hedef sunucuyu güncelleştirir.

### [Güncelleştirme-AzureRmSiteRecoveryRecoveryPlan](Update-AzureRmSiteRecoveryRecoveryPlan.md)
Site kurtarma 'da kurtarma planını güncelleştirir.

### [Güncelleştirme-AzureRmSiteRecoveryServer](Update-AzureRmSiteRecoveryServer.md)
Site kurtarma sunucusunu yeniler.

### [Güncelleştirme-AzureRmSiteRecoveryServicesProvider](Update-AzureRmSiteRecoveryServicesProvider.md)
Azure Site Recovery Hizmetleri sağlayıcısından alınan bilgileri güncelleştirir.

